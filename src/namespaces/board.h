...
class board {
 ...
 public:
  ...
  static u64 __attribute__((always_inline)) getPinned(const u64 allpieces, const u64 friends, const int kingPosition,
    ...
    u64 attacked =
        DIAGONAL_ANTIDIAGONAL[kingPosition] & (chessboard[QUEEN_BLACK + xside] | chessboard[BISHOP_BLACK + xside]);
  ...
  __attribute__((always_inline)) static u64 getBitmap(const _Tchessboard &chessboard) {
    return chessboard[PAWN_BLACK] | chessboard[ROOK_BLACK] | chessboard[BISHOP_BLACK] | chessboard[KNIGHT_BLACK] |
           chessboard[KING_BLACK] | chessboard[QUEEN_BLACK] | chessboard[PAWN_WHITE] | chessboard[ROOK_WHITE] |
           chessboard[BISHOP_WHITE] | chessboard[KNIGHT_WHITE] | chessboard[KING_WHITE] | chessboard[QUEEN_WHITE];
  }
  ...
  template <uchar side>
  __attribute__((always_inline)) static u64 getAttackers(const int position, const u64 allpieces,
                                                         const _Tchessboard &chessboard) {
  ...
  template <uchar side>
  __attribute__((always_inline)) static bool isAttacked(const int position, const u64 allpieces,
                                                        const _Tchessboard &chessboard) {
