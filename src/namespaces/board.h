...
class board {
  ...
  public:
    ...
    template <uchar side> __attribute__((always_inline)) static u64 getBitmap(const _Tchessboard &chessboard) {
    ...
    template <uchar side> __attribute__((always_inline)) static int getPieceAt(const u64 bitmapPos, const _Tchessboard &chessboard) {
    ...
    static u64 getBitmap(uchar side, const _Tchessboard &chessboard);

    static int getPieceAt(uchar side, u64 bitmapPos, const _Tchessboard &chessboard);
    ...
    template <uchar side> __attribute__((always_inline)) static bool anyAttack(u64 sq, const u64 allpieces, const _Tchessboard &chessboard) {
        for (; sq; RESET_LSB(sq)) {
    ...
    template <uchar side> __attribute__((always_inline)) static bool inCheck1(const _Tchessboard &chessboard) {
        return isAttacked<side>(BITScanForward(chessboard[KING_BLACK + side]), getBitmap<BLACK>(chessboard) | getBitmap<WHITE>(chessboard),
                                chessboard);
    }

    template <uchar side> __attribute__((always_inline)) static u64 getBitmapNoPawnsNoKing(const _Tchessboard &chessboard) {
        BENCH_AUTO_CLOSE("getBitmapNoPawns");
        return chessboard[ROOK_BLACK + side] | chessboard[BISHOP_BLACK + side] | chessboard[KNIGHT_BLACK + side] | chessboard[QUEEN_BLACK + side];
    }

    template <uchar side> __attribute__((always_inline)) static u64 getPiecesNoKing(const _Tchessboard &chessboard) {
        BENCH_AUTO_CLOSE("getPiecesNoKing");
        return chessboard[ROOK_BLACK + side] | chessboard[BISHOP_BLACK + side] | chessboard[KNIGHT_BLACK + side] | chessboard[PAWN_BLACK + side] |
