...
class See {
  public:
    static int see(const _Tmove &move, const _Tchessboard &chessboard, const u64 allpieces) {
        ...
        if (yourCount < myCount) {
            int score = 0;
            for (int j = 0; j < yourCount; j++)
                score += yourCap[j];
            for (int j = 0; j < myCount - 1; j++)
                score -= myCap[j];
            return score + pieceValue;
        }
        return pieceValue;
  ...
  private:
    static int _see(const uchar side, const int position, const u64 allpieces, int attackersValue[], const int firstValueFrom,
