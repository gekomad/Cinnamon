...
// class See {
// public:
//     static int see(const _Tmove &move, const _Tchessboard &chessboard, const
//     u64 allpieces) {
//         const uchar side = move.side;
//         const int position = move.to;
//         const int capturedPiece = move.capturedPiece;
//         ASSERT(allpieces == (board::getBitmap<WHITE>(chessboard) |
//         board::getBitmap<BLACK>(chessboard))); int yourCap[15]; int yourCount
//         = _see(side, position, allpieces, yourCap,
//         PIECES_VALUE[move.pieceFrom], chessboard); const int pieceValue =
//         PIECES_VALUE[capturedPiece]; if (yourCount == 0) return pieceValue;
//         int myCap[15];
//         int myCount = _see(X(side), position, allpieces, myCap,
//         PIECES_VALUE[move.pieceFrom], chessboard);
//
...
// private:
//     static int
//     _see(const uchar side, const int position, const u64 allpieces, int
//     attackersValue[], const int firstValueFrom,
...
//         ///pawn
//         u64 attackers = PAWN_FORK_MASK[side][position] &
//         chessboard[PAWN_BLACK + xside]; for (; attackers;
//         RESET_LSB(attackers)) {
...
//             (chessboard[BISHOP_BLACK + xside] | chessboard[ROOK_BLACK +
//             xside] |
//              chessboard[QUEEN_BLACK + xside])) {
//             ///bishop
//             auto diagAnt = Bitboard::getDiagonalAntiDiagonal(position,
//             allpieces); u64 b = diagAnt & chessboard[BISHOP_BLACK + xside];
//             for (; b; RESET_LSB(b)) {
