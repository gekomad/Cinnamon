...
int Eval::evaluatePawn() {
    ...
    const u64 ped_friends = chessboard.bit[side];
    const u64 ped_enemies = chessboard.bit[side ^ 1];
    // 1.
    if (!ped_friends) {
    ...
    // 2.
    int result = MOB_PAWNS[getMobilityPawns<side>(chessboard.bit[ENPASSANT_IDX], ped_friends,
    ...
    if (bitCount(chessboard.bit[side ^ 1]) == 8) {
        result -= ENEMIES_ALL_PAWNS;
        ADD(SCORE_DEBUG.ENEMIES_PAWNS_ALL[side], -ENEMIES_ALL_PAWNS);
    }
...
//    }
    ...
    if (phase != OPEN) {
        ...
        structureEval.kingSecurityDistance[side] -=
                ENEMY_NEAR_KING * bitCount(NEAR_MASK2[structureEval.posKing[side]] & ped_enemies);
    }
    ...
    if (phase != OPEN) {
        ...
        const u64 pawnsIn8 = (shiftForward<side, 8>(pawnsIn7) & (~structureEval.allPieces) |
                              structureEval.allPiecesSide[side ^ 1] &
                              (shiftForward<side, 7>(pawnsIn7) | shiftForward<side, 9>(pawnsIn7)) & RANK_1_8[side]
        );
        result += PAWN_IN_RACE * (bitCount(pawnsIn8));
        ADD(SCORE_DEBUG.PAWN_IN_RACE[side], PAWN_IN_RACE * (bitCount(pawnsIn8)));
    }

    // 9. unprotected TODO verificare
    const u64 unpotectedPawn = (shiftForward<side, 7>(ped_friends) & NO_FILE_LEFT[side] |
                                shiftForward<side, 9>(ped_friends) & NO_FILE_RIGHT[side]) & (~ped_friends);

    result -= UNPROTECTED_PAWNS * bitCount(unpotectedPawn);
...
int Eval::evaluateBishop(const u64 enemies) {
    INC(evaluationCount[side]);
    u64 bishop = chessboard.bit[BISHOP_BLACK + side];
...
int Eval::evaluateKnight(const u64 enemiesPawns, const u64 notMyBits) {
    INC(evaluationCount[side]);
    u64 knight = chessboard.bit[KNIGHT_BLACK + side];
    //if (!x) return 0;TODO
...
template<int side, Eval::_Tphase phase>
int Eval::evaluateKing(const u64 notMyBits) {
    ...
    //mobility
    ASSERT(bitCount(notMyBits & NEAR_MASK1[pos_king]) < (int) (sizeof(MOB_KING[phase]) / sizeof(int)));
    result += MOB_KING[phase][bitCount(notMyBits & NEAR_MASK1[pos_king])];
    ADD(SCORE_DEBUG.MOB_KING[side], MOB_KING[phase][bitCount(notMyBits & NEAR_MASK1[pos_king])]);
    if (phase != OPEN) {
...
//    int endGameValue; TODO utilizzare?
...
//    }
    ...
    if (trace) {
        ...
        if (side == WHITE) cout << "\nTotal (white)..........   " << (double) -result / 100.0 << endl;
        else
            cout << endl << "Total (black)..........   " << (double) result / 100.0 << endl;
        cout << "PHASE: ";
        if (phase == OPEN) {
            cout << " OPEN" << endl;
...
//        cout << "Semi-open file:   " << setw(10) << (double) (SCORE_DEBUG.HALF_OPEN_FILE[WHITE] - SCORE_DEBUG.HALF_OPEN_FILE[BLACK]) / 100.0 << setw(15) << (double) (SCORE_DEBUG.HALF_OPEN_FILE[WHITE]) / 100.0 << setw(10) << (double) (SCORE_DEBUG.HALF_OPEN_FILE[BLACK]) / 100.0 << endl;
        cout << "Mobility:         " << setw(10) << (double) (mobWhite - mobBlack) / 100.0 << setw(15) <<
        (double) (mobWhite) / 100.0 << setw(10) << (double) (mobBlack) / 100.0 << endl;
//        cout << "Attack king:      " << setw(10) << (double) (attack_king_white - attack_king_black) / 100.0 << setw(15) << (double) (attack_king_white) / 100.0 << setw(10) << (double) (attack_king_black) / 100.0 << endl;
        cout << "Bonus attack king:" << setw(10) <<
        (double) (bonus_attack_king_white - bonus_attack_king_black) / 100.0 << setw(15) <<
        (double) (bonus_attack_king_white) / 100.0 << setw(10) << (double) (bonus_attack_king_black) / 100.0 << endl;
        ...
        setw(10) << (double) (SCORE_DEBUG.MOB_PAWNS[BLACK]) / 100.0 << endl;
        cout << "       attack king:              " << setw(10) <<
        (double) (SCORE_DEBUG.ATTACK_KING_PAWN[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.ATTACK_KING_PAWN[BLACK]) / 100.0 << endl;
        cout << "       center:                   " << setw(10) << (double) (SCORE_DEBUG.PAWN_CENTER[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.PAWN_CENTER[BLACK]) / 100.0 << endl;
        cout << "       7h:                       " << setw(10) << (double) (SCORE_DEBUG.PAWN_7H[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.PAWN_7H[BLACK]) / 100.0 << endl;
        cout << "       in race:                  " << setw(10) << (double) (SCORE_DEBUG.PAWN_IN_RACE[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.PAWN_IN_RACE[BLACK]) / 100.0 << endl;
        cout << "       blocked:                  " << setw(10) << (double) (SCORE_DEBUG.PAWN_BLOCKED[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.PAWN_BLOCKED[BLACK]) / 100.0 << endl;
        cout << "       unprotected:              " << setw(10) <<
        (double) (SCORE_DEBUG.UNPROTECTED_PAWNS[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.UNPROTECTED_PAWNS[BLACK]) / 100.0 << endl;
        cout << "       isolated                  " << setw(10) <<
        (double) (SCORE_DEBUG.PAWN_ISOLATED[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.PAWN_ISOLATED[BLACK]) / 100.0 << endl;
        cout << "       double                    " << setw(10) <<
        (double) (SCORE_DEBUG.DOUBLED_PAWNS[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.DOUBLED_PAWNS[BLACK]) / 100.0 << endl;
        cout << "       double isolated           " << setw(10) <<
        (double) (SCORE_DEBUG.DOUBLED_ISOLATED_PAWNS[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.DOUBLED_ISOLATED_PAWNS[BLACK]) / 100.0 << endl;
        cout << "       backward                  " << setw(10) <<
        (double) (SCORE_DEBUG.BACKWARD_PAWN[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.BACKWARD_PAWN[BLACK]) / 100.0 << endl;
        cout << "       fork:                     " << setw(10) << (double) (SCORE_DEBUG.FORK_SCORE[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.FORK_SCORE[BLACK]) / 100.0 << endl;
        cout << "       passed:                   " << setw(10) << (double) (SCORE_DEBUG.PAWN_PASSED[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.PAWN_PASSED[BLACK]) / 100.0 << endl;
        cout << "       all enemies:              " << setw(10) <<
        (double) (SCORE_DEBUG.ENEMIES_PAWNS_ALL[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.ENEMIES_PAWNS_ALL[BLACK]) / 100.0 << endl;
        cout << "       none:                     " << setw(10) << (double) (SCORE_DEBUG.NO_PAWNS[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.NO_PAWNS[BLACK]) / 100.0 << endl;

        cout << HEADER;
        cout << "Knight:           " << setw(10) <<
        (double) (Tresult.knights[WHITE] - Tresult.knights[BLACK]) / 100.0 << setw(15) <<
        (double) (Tresult.knights[WHITE]) / 100.0 << setw(10) << (double) (Tresult.knights[BLACK]) / 100.0 << endl;
        cout << "       undevelop:                " << setw(10) <<
        (double) (SCORE_DEBUG.UNDEVELOPED_KNIGHT[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.UNDEVELOPED_KNIGHT[BLACK]) / 100.0 << endl;
        cout << "       trapped:                  " << setw(10) <<
        (double) (SCORE_DEBUG.KNIGHT_TRAPPED[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.KNIGHT_TRAPPED[BLACK]) / 100.0 << endl;
        cout << "       mobility:                 " << setw(10) << (double) (SCORE_DEBUG.MOB_KNIGHT[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.MOB_KNIGHT[BLACK]) / 100.0 << endl;
//        cout << "       near enemy king           " << setw(10) << (double) (SCORE_DEBUG.XKNIGHT_NEAR_KING[WHITE]) / 100.0 << setw(10) << (double) (SCORE_DEBUG.XKNIGHT_NEAR_KING[BLACK]) / 100.0 << endl;
        ...
        setw(10) << (double) (SCORE_DEBUG.BAD_BISHOP[BLACK]) / 100.0 << endl;
        cout << "       mobility:                 " << setw(10) << (double) (SCORE_DEBUG.MOB_BISHOP[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.MOB_BISHOP[BLACK]) / 100.0 << endl;
        cout << "       undevelop:                " << setw(10) <<
        (double) (SCORE_DEBUG.UNDEVELOPED_BISHOP[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.UNDEVELOPED_BISHOP[BLACK]) / 100.0 << endl;
        cout << "       open diag:                " << setw(10) <<
        (double) (SCORE_DEBUG.OPEN_DIAG_BISHOP[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.OPEN_DIAG_BISHOP[BLACK]) / 100.0 << endl;
        cout << "       bonus 2 bishops:          " << setw(10) << (double) (SCORE_DEBUG.BONUS2BISHOP[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.BONUS2BISHOP[BLACK]) / 100.0 << endl;
//        cout << "       near enemy king           " << setw(10) << (double) (SCORE_DEBUG.XBISHOP_NEAR_KING[WHITE]) / 100.0 << setw(10) << (double) (SCORE_DEBUG.XBISHOP_NEAR_KING[BLACK]) / 100.0 << endl;
        ...
        (double) (SCORE_DEBUG.ROOK_7TH_RANK[BLACK]) / 100.0 << endl;
        cout << "       trapped:                  " << setw(10) << (double) (SCORE_DEBUG.ROOK_TRAPPED[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.ROOK_TRAPPED[BLACK]) / 100.0 << endl;
        cout << "       mobility:                 " << setw(10) << (double) (SCORE_DEBUG.MOB_ROOK[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.MOB_ROOK[BLACK]) / 100.0 << endl;
        cout << "       blocked:                  " << setw(10) << (double) (SCORE_DEBUG.ROOK_BLOCKED[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.ROOK_BLOCKED[BLACK]) / 100.0 << endl;
        cout << "       open file:                " << setw(10) <<
        (double) (SCORE_DEBUG.ROOK_OPEN_FILE[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.ROOK_OPEN_FILE[BLACK]) / 100.0 << endl;
//        cout << "       semi open file:           " << setw(10) << (double) (SCORE_DEBUG.ROOK_SEMI_OPEN_FILE[WHITE]) / 100.0 << setw(10) << (double) (SCORE_DEBUG.ROOK_SEMI_OPEN_FILE[BLACK]) / 100.0 << endl;
        cout << "       connected:                " << setw(10) <<
        (double) (SCORE_DEBUG.CONNECTED_ROOKS[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.CONNECTED_ROOKS[BLACK]) / 100.0 << endl;
//        cout << "       near enemy king           " << setw(10) << (double) (SCORE_DEBUG.XROOK_NEAR_KING[WHITE]) / 100.0 << setw(10) << (double) (SCORE_DEBUG.XROOK_NEAR_KING[BLACK]) / 100.0 << endl;
        ...
        setw(10) << (double) (SCORE_DEBUG.MOB_QUEEN[BLACK]) / 100.0 << endl;
        cout << "       bishop on queen:          " << setw(10) <<
        (double) (SCORE_DEBUG.BISHOP_ON_QUEEN[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.BISHOP_ON_QUEEN[BLACK]) / 100.0 << endl;
//        cout << "       near enemy king           " << setw(10) << (double) (SCORE_DEBUG.XQUEEN_NEAR_KING[WHITE]) / 100.0 << setw(10) << (double) (SCORE_DEBUG.XQUEEN_NEAR_KING[BLACK]) / 100.0 << endl;
        ...
        (double) (SCORE_DEBUG.DISTANCE_KING[BLACK]) / 100.0 << endl;
        cout << "       open file:                " << setw(10) <<
        (double) (SCORE_DEBUG.END_OPENING_KING[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.END_OPENING_KING[BLACK]) / 100.0 << endl;
        cout << "       pawn near:                " << setw(10) <<
        (double) (SCORE_DEBUG.PAWN_NEAR_KING[WHITE]) / 100.0 << setw(10) <<
        (double) (SCORE_DEBUG.PAWN_NEAR_KING[BLACK]) / 100.0 << endl;
//        cout << "       pawn storm:               " << setw(10) << (double) (SCORE_DEBUG.PAWN_STORM[WHITE]) / 100.0 << setw(10) << (double) (SCORE_DEBUG.PAWN_STORM[BLACK]) / 100.0 << endl;
        cout << "       mobility:                 " << setw(10) << (double) (SCORE_DEBUG.MOB_KING[WHITE]) / 100.0 <<
        setw(10) << (double) (SCORE_DEBUG.MOB_KING[BLACK]) / 100.0 << endl;
//        cout << "       bishop near king:         " << setw(10) << (double) (SCORE_DEBUG.BISHOP_NEAR_KING[WHITE]) / 100.0 << setw(10) << (double) (SCORE_DEBUG.BISHOP_NEAR_KING[BLACK]) / 100.0 << endl;
//        cout << "       queen near king:          " << setw(10) << (double) (SCORE_DEBUG.QUEEN_NEAR_KING[WHITE]) / 100.0 << setw(10) << (double) (SCORE_DEBUG.QUEEN_NEAR_KING[BLACK]) / 100.0 << endl;
//        cout << "       knight near king:         " << setw(10) << (double) (SCORE_DEBUG.KNIGHT_NEAR_KING[WHITE]) / 100.0 << setw(10) << (double) (SCORE_DEBUG.KNIGHT_NEAR_KING[BLACK]) / 100.0 << endl;
//        cout << "       rook near king:           " << setw(10) << (double) (SCORE_DEBUG.ROOK_NEAR_KING[WHITE]) / 100.0 << setw(10) << (double) (SCORE_DEBUG.ROOK_NEAR_KING[BLACK]) / 100.0 << endl;
        cout << endl;
    }
#endif
