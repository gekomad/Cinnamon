 ...
 * 7. *king security* - in OPEN phase add at kingSecurity FRIEND_NEAR_KING * each pawn near to king and substracts
 ENEMY_NEAR_KING * each enemy pawn near to king
 ...
 * 12. doubled - there aren't friend pawns on the two sides - subtracts DOUBLED_PAWNS for each pawn. If it is isolated
 too substracts DOUBLED_ISOLATED_PAWNS
...
int Eval::evaluatePawn(const _Tchessboard &chessboard) {
  ...
  if (phase != OPEN) {
    ...
    const u64 pawnsIn8 =
        (shiftForward<side, 8>(pawnsIn7) & (~structureEval.allPieces)) |
 ...
 * 2. *king security* - in OPEN phase add at kingSecurity FRIEND_NEAR_KING for each queen near to king and substracts
 * ENEMY_NEAR_KING for each queen near to enemy king
 ...
 * 4. *king security* - in OPEN phase add at kingSecurity FRIEND_NEAR_KING for each knight near to king and substracts
 * ENEMY_NEAR_KING for each knight near to enemy king
 * 5. mobility
 * 6. outposts
 */
 ...
 * 4. *king security* - in OPEN phase add at kingSecurity FRIEND_NEAR_KING for each rook near to king and substracts
 * ENEMY_NEAR_KING for each rook near to enemy king
...
int Eval::evaluateRook(const _Tchessboard &chessboard, const u64 enemies, const u64 friends) {
  ...
  // .6
  //    if (((F1G1bit[side] & king) && (H1H2G1bit[side] & rook)) || ((C1B1bit[side] & king) && (A1A2B1bit[side] &
  //    rook))) {
...
short Eval::getScore(const _Tchessboard &chessboard, const u64 key, const uchar side, const int alpha,
                     const int beta DEBUG2(, const bool trace)) {
  /// endgame
...
//    }
  ...
  if (phase != OPEN) {
    structureEval.pinned[WHITE] = board::getPinned<WHITE>(structureEval.allPieces, structureEval.allPiecesSide[WHITE],
                                                          structureEval.posKing[WHITE], chessboard);
    structureEval.pinned[BLACK] = board::getPinned<BLACK>(structureEval.allPieces, structureEval.allPiecesSide[BLACK],
  ...
  const int result =
      (attack_king_black + bonus_attack_king_black + lazyscore_black + Tresult.pawns[BLACK] + Tresult.knights[BLACK] +
