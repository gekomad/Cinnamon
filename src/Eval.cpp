...
//                ENEMY_NEAR_KING * bitCount(NEAR_MASK2[structureEval.posKing[xside]] & ped_friends);
    ...
    ADD(SCORE_DEBUG[MG].PAWN_7H[side], PAWN_IN_7TH[MG] * bitCount(pawnsIn7));
    ADD(SCORE_DEBUG[EG].PAWN_7H[side], PAWN_IN_7TH[EG] * bitCount(pawnsIn7));
...
pair<int, int> Eval::evaluateKing(const _Tchessboard &chessboard, const uchar side, const u64 squares) {
    ...
    ADD(SCORE_DEBUG[MG].DISTANCE_KING[side], DISTANCE_KING_ENDING[pos_king]);
    ...
    //mobility
    assert(bitCount(squares & NEAR_MASK1[pos_king]) < (int) (sizeof(MOB_KING[MG]) / sizeof(int)));
...
//    phase = (phase * 256 + (TotalPhase / 2)) / TotalPhase;
    ...
    if (trace) {
        ...
        cout << "Material:         " << setw(10) << (float) (lazyscore_white - lazyscore_black) / 100.0 << setw(15)
             << (float) (lazyscore_white) / 100.0 << setw(10) << (float) (lazyscore_black) / 100.0 << endl;
        ...
        p(SCORE_DEBUG[EG].MOB_QUEEN[WHITE], SCORE_DEBUG[EG].MOB_QUEEN[BLACK]);

        cout << endl << "bishop on queen\t";
        p();
        p(SCORE_DEBUG[MG].BISHOP_ON_QUEEN[WHITE], SCORE_DEBUG[MG].BISHOP_ON_QUEEN[BLACK]);
        p(SCORE_DEBUG[EG].BISHOP_ON_QUEEN[WHITE], SCORE_DEBUG[EG].BISHOP_ON_QUEEN[BLACK]);

        cout << endl << "\tKING\t|";
        p((tResult[MG].kings[WHITE] - tResult[MG].kings[BLACK]), (tResult[EG].kings[WHITE] - tResult[EG].kings[BLACK]));
        p(tResult[MG].kings[WHITE], tResult[EG].kings[BLACK]);
        p(tResult[EG].kings[BLACK], tResult[EG].kings[BLACK]);
        cout << endl << "distance\t";
        p();
        p(SCORE_DEBUG[MG].DISTANCE_KING[WHITE], SCORE_DEBUG[MG].DISTANCE_KING[BLACK]);
        p(SCORE_DEBUG[EG].DISTANCE_KING[WHITE], SCORE_DEBUG[EG].DISTANCE_KING[BLACK]);
