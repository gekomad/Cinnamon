...
#ifdef TUNING
    PARAMS["MOB_BISHOP_INC"] = PARAM(&MOB_BISHOP_INC);
    PARAMS["PAWN_PASSED_INC"] = PARAM(&PAWN_PASSED_INC);
    PARAMS["MOB_BISHOP_INC"] = PARAM(&MOB_BISHOP_INC);
    PARAMS["MOB_QUEEN_INC"] = PARAM(&MOB_QUEEN_INC);
    PARAMS["DISTANCE_KING_ENDING_INC"] = &DISTANCE_KING_ENDING_INC;
    PARAMS["MOB_KING_INC"] = &MOB_KING_INC;
    PARAMS["DISTANCE_KING_OPENING_INC"] = &DISTANCE_KING_OPENING_INC;
    PARAMS["MOB_ROOK_INC"] = &MOB_ROOK_INC;
    PARAMS["MOB_KNIGHT_INC"] = &MOB_KNIGHT_INC;
    PARAMS["PHASE_END"] = &PHASE_END;
    PARAMS["PHASE_MIDDLE"] = &PHASE_MIDDLE;
    PARAMS["REVERSE_FUTIL_MARGIN"] = &REVERSE_FUTIL_MARGIN;
    PARAMS["EXT_FUTIL_MARGIN"] = &EXT_FUTIL_MARGIN;
    PARAMS["FUTIL_MARGIN"] = &FUTIL_MARGIN;
    PARAMS["ATTACK_KING"] = &ATTACK_KING;
    PARAMS["BISHOP_ON_QUEEN"] = &BISHOP_ON_QUEEN;
    PARAMS["BACKWARD_PAWN"] = &BACKWARD_PAWN;
    PARAMS["DOUBLED_ISOLATED_PAWNS"] = &DOUBLED_ISOLATED_PAWNS;
    PARAMS["PAWN_IN_7TH"] = &PAWN_IN_7TH;
    PARAMS["PAWN_IN_PROMOTION"] = &PAWN_IN_PROMOTION;
    PARAMS["PAWN_NEAR_KING"] = &PAWN_NEAR_KING;
    PARAMS["PAWN_BLOCKED"] = &PAWN_BLOCKED;
    PARAMS["UNPROTECTED_PAWNS"] = &UNPROTECTED_PAWNS;
    PARAMS["FRIEND_NEAR_KING"] = &FRIEND_NEAR_KING;
    PARAMS["BONUS2BISHOP"] = &BONUS2BISHOP;
    PARAMS["BISHOP_PAWN_ON_SAME_COLOR"] = &BISHOP_PAWN_ON_SAME_COLOR;
    PARAMS["OPEN_FILE_Q"] = &OPEN_FILE_Q;
    PARAMS["ROOK_7TH_RANK"] = &ROOK_7TH_RANK;
    PARAMS["KNIGHT_PINNED"] = &KNIGHT_PINNED;
    PARAMS["ROOK_PINNED"] = &ROOK_PINNED;
    PARAMS["BISHOP_PINNED"] = &BISHOP_PINNED;
    PARAMS["QUEEN_PINNED"] = &QUEEN_PINNED;
    PARAMS["QUEEN_IN_7"] = &QUEEN_IN_7;
    PARAMS["ROOK_IN_7"] = &ROOK_IN_7;
#endif
}
 ...
 * 7. *king security* - in OPEN phase add at kingSecurity FRIEND_NEAR_KING * each pawn near to king and substracts ENEMY_NEAR_KING * each enemy pawn
 near to king
 ...
 * 12. doubled - there aren't friend pawns on the two sides - subtracts DOUBLED_PAWNS for each pawn. If it is isolated too substracts
 ...
 */
template <uchar side, Eval::_Tphase phase> int Eval::evaluatePawn(const _Tchessboard &chessboard) {
    ...
    if (phase != OPEN) {
        ...
        structureEval.kingSecurity[side] += FRIEND_NEAR_KING * bitCount(NEAR_MASK2[structureEval.posKing[side]] & ped_friends);
    ...
    for (u64 p = ped_friends; p; RESET_LSB(p)) {
        ...
        /// blocked
        result -= (!(PAWN_FORK_MASK[side][o] & structureEval.allPiecesSide[xside])) && (structureEval.allPieces & (shiftForward<side, 8>(pos)))
                      ? PAWN_BLOCKED
                      : 0;
        ADD(SCORE_DEBUG.PAWN_BLOCKED[side],
            (!(PAWN_FORK_MASK[side][o] & structureEval.allPiecesSide[xside])) && (structureEval.allPieces & (shiftForward<side, 8>(pos)))
                ? -PAWN_BLOCKED
                : 0);
        /// unprotected
        ...
        if (!(chessboard[xside] & PAWN_PASSED_MASK[side][o])) {
            ...
            result += PAWN_PASSED[side][o] + PAWN_PASSED_INC;
        }
    }
    return result;
}
 ...
 * 9. pinned
 */
template <uchar side, Eval::_Tphase phase> int Eval::evaluateBishop(const _Tchessboard &chessboard, const u64 enemies) {
    INC(evaluationCount[side]);
    constexpr int xside = X(side);
    ...
    for (; bishop; RESET_LSB(bishop)) {
        ...
        assert(bitCount(captured) + bitCount(x & ~structureEval.allPieces) < static_cast<int>(sizeof(MOB_BISHOP) / sizeof(int)));
        if (captured & structureEval.posKingBit[xside]) structureEval.kingAttackers[xside] |= POW2(o);

        result += MOB_BISHOP[phase][bitCount(captured) + bitCount(x & ~structureEval.allPieces)] + MOB_BISHOP_INC;

        ADD(SCORE_DEBUG.MOB_BISHOP[side], MOB_BISHOP[phase][bitCount(captured) + bitCount(x & ~structureEval.allPieces)]);
 ...
 * 2. *king security* - in OPEN phase add at kingSecurity FRIEND_NEAR_KING for each queen near to king and substracts ENEMY_NEAR_KING for each queen
 * near to enemy king
 ...
 */
template <uchar side, Eval::_Tphase phase> int Eval::evaluateQueen(const _Tchessboard &chessboard, const u64 enemies) {
    INC(evaluationCount[side]);
    ...
    if (phase != OPEN) {
        ...
        structureEval.kingSecurity[side] += FRIEND_NEAR_KING * bitCount(NEAR_MASK2[structureEval.posKing[side]] & queen);
        ADD(SCORE_DEBUG.KING_SECURITY_QUEEN[side], FRIEND_NEAR_KING * bitCount(NEAR_MASK2[structureEval.posKing[side]] & queen));
    ...
    for (; queen; RESET_LSB(queen)) {
        ...
        result += MOB_QUEEN[phase][bitCount(x)] + MOB_QUEEN_INC;
 ...
 * 4. *king security* - in OPEN phase add at kingSecurity FRIEND_NEAR_KING for each knight near to king and substracts ENEMY_NEAR_KING for each knight
 * near to enemy king
 * 5. mobility
 * 6. outposts
 */

template <uchar side, Eval::_Tphase phase> int Eval::evaluateKnight(const _Tchessboard &chessboard, const u64 notMyBits) {
    INC(evaluationCount[side]);
    ...
    if (phase != OPEN) {
        ...
        structureEval.kingSecurity[side] += FRIEND_NEAR_KING * bitCount(NEAR_MASK2[structureEval.posKing[side]] & knight);
        ADD(SCORE_DEBUG.KING_SECURITY_KNIGHT[side], FRIEND_NEAR_KING * bitCount(NEAR_MASK2[structureEval.posKing[side]] & knight));
    ...
    for (; knight; RESET_LSB(knight)) {
        ...
        result += MOB_KNIGHT[bitCount(mob)] + MOB_KNIGHT_INC;
        ...
        if (p && !(PAWN_FORK_MASK[xside][pos] & chessboard[xside])) {
            ...
            if (PAWN_FORK_MASK[xside][pos] & chessboard[side]) {
                result += p;
                if (!(chessboard[KNIGHT_BLACK + xside]) && !(chessboard[BISHOP_BLACK + xside] & board::colors(pos))) {
                    result += p;
                }
            }
        }
 ...
 * 4. *king security* - in OPEN phase add at kingSecurity FRIEND_NEAR_KING for each rook near to king and substracts ENEMY_NEAR_KING for each rook
 * near to enemy king
 ...
 */
template <uchar side, Eval::_Tphase phase> int Eval::evaluateRook(const _Tchessboard &chessboard, const u64 enemies, const u64 friends) {
    INC(evaluationCount[side]);
    ...
    if (phase != OPEN) {
        ...
        structureEval.kingSecurity[side] += FRIEND_NEAR_KING * bitCount(NEAR_MASK2[structureEval.posKing[side]] & rook);
        ADD(SCORE_DEBUG.KING_SECURITY_ROOK[side], FRIEND_NEAR_KING * bitCount(NEAR_MASK2[structureEval.posKing[side]] & rook));
    ...
    for (; rook; RESET_LSB(rook)) {
        ...
        result += MOB_ROOK[phase][bitCount(mob)] + MOB_ROOK_INC;
...
template <Eval::_Tphase phase> int Eval::evaluateKing(const _Tchessboard &chessboard, const uchar side, const u64 squares) {
    ...
    if (phase == END) {
        ...
        result = DISTANCE_KING_ENDING[pos_king] + DISTANCE_KING_ENDING_INC;
    } else {
        ADD(SCORE_DEBUG.DISTANCE_KING[side], DISTANCE_KING_OPENING[pos_king]);
        result = DISTANCE_KING_OPENING[pos_king] + DISTANCE_KING_OPENING_INC;
    }

    // mobility
    assert(bitCount(squares & NEAR_MASK1[pos_king]) < static_cast<int>(sizeof(MOB_KING[phase]) / sizeof(int)));
    result += MOB_KING[phase][bitCount(squares & NEAR_MASK1[pos_king])] + MOB_KING_INC;
...
short Eval::getScore(const _Tchessboard &chessboard, const u64 key, const uchar side, const int alpha, const int beta DEBUG2(, const bool trace)) {
    /// endgame
...
//    }
    ...
    _Tphase phase;
    if (npieces < PHASE_END) phase = END;
    else if (npieces < PHASE_MIDDLE)
        phase = MIDDLE;
    else
        phase = OPEN;
    ...
    if (phase != OPEN) {
        structureEval.pinned[WHITE] =
            board::getPinned<WHITE>(structureEval.allPieces, structureEval.allPiecesSide[WHITE], structureEval.posKing[WHITE], chessboard);
        structureEval.pinned[BLACK] =
            board::getPinned<BLACK>(structureEval.allPieces, structureEval.allPiecesSide[BLACK], structureEval.posKing[BLACK], chessboard);
    } else {
    ...
    const int bonus_attack_king_black = phase == OPEN ? 0 : BONUS_ATTACK_KING[bitCount(structureEval.kingAttackers[WHITE])] + BONUS_ATTACK_KING_INC;
    const int bonus_attack_king_white = phase == OPEN ? 0 : BONUS_ATTACK_KING[bitCount(structureEval.kingAttackers[BLACK])] + BONUS_ATTACK_KING_INC;
    ...
    const int result = (attack_king_black + bonus_attack_king_black + lazyscore_black + Tresult.pawns[BLACK] + Tresult.knights[BLACK] +
                        Tresult.bishop[BLACK] + Tresult.rooks[BLACK] + Tresult.queens[BLACK] + Tresult.kings[BLACK]) -
                       (attack_king_white + bonus_attack_king_white + lazyscore_white + Tresult.pawns[WHITE] + Tresult.knights[WHITE] +
    ...
    if (trace) {
        ...
        cout << HEADER;
        cout << "|Material:         " << setw(10) << static_cast<double>(lazyscore_white - lazyscore_black) / 100.0 << setw(15)
             << static_cast<double>(lazyscore_white) / 100.0 << setw(10) << static_cast<double>(lazyscore_black) / 100.0 << "\n";
        cout << "|Bonus attack king:" << setw(10) << static_cast<double>(bonus_attack_king_white - bonus_attack_king_black) / 100.0 << setw(15)
             << static_cast<double>(bonus_attack_king_white) / 100.0 << setw(10) << static_cast<double>(bonus_attack_king_black) / 100.0 << "\n";

        cout << HEADER;
        cout << "|Pawn:             " << setw(10) << static_cast<double>(Tresult.pawns[WHITE] - Tresult.pawns[BLACK]) / 100.0 << setw(15)
             << static_cast<double>(Tresult.pawns[WHITE]) / 100.0 << setw(10) << static_cast<double>(Tresult.pawns[BLACK]) / 100.0 << "\n";
        cout << "|       attack king:              " << setw(10) << static_cast<double>(SCORE_DEBUG.ATTACK_KING_PAWN[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.ATTACK_KING_PAWN[BLACK]) / 100.0 << "\n";
        //        cout << "|       center:                   " << setw(10) << static_cast<double> (SCORE_DEBUG.PAWN_CENTER[WHITE]) / 100.0 <<
        //             setw(10) << static_cast<double> (SCORE_DEBUG.PAWN_CENTER[BLACK]) / 100.0 << "\n";
        cout << "|       in 7th:                   " << setw(10) << static_cast<double>(SCORE_DEBUG.PAWN_7H[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.PAWN_7H[BLACK]) / 100.0 << "\n";
        cout << "|       in promotion:             " << setw(10) << static_cast<double>(SCORE_DEBUG.PAWN_IN_PROMOTION[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.PAWN_IN_PROMOTION[BLACK]) / 100.0 << "\n";
        cout << "|       blocked:                  " << setw(10) << static_cast<double>(SCORE_DEBUG.PAWN_BLOCKED[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.PAWN_BLOCKED[BLACK]) / 100.0 << "\n";
        cout << "|       unprotected:              " << setw(10) << static_cast<double>(SCORE_DEBUG.UNPROTECTED_PAWNS[WHITE]) / 100.0 << setw(10)
        ...
        cout << "|       double isolated           " << setw(10) << static_cast<double>(SCORE_DEBUG.DOUBLED_ISOLATED_PAWNS[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.DOUBLED_ISOLATED_PAWNS[BLACK]) / 100.0 << "\n";
        cout << "|       backward                  " << setw(10) << static_cast<double>(SCORE_DEBUG.BACKWARD_PAWN[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.BACKWARD_PAWN[BLACK]) / 100.0 << "\n";
        cout << "|       fork:                     " << setw(10) << static_cast<double>(SCORE_DEBUG.FORK_SCORE[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.FORK_SCORE[BLACK]) / 100.0 << "\n";
        cout << "|       passed:                   " << setw(10) << static_cast<double>(SCORE_DEBUG.PAWN_PASSED[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.PAWN_PASSED[BLACK]) / 100.0 << "\n";
        cout << "|       all enemies:              " << setw(10) << static_cast<double>(SCORE_DEBUG.ENEMIES_PAWNS_ALL[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.ENEMIES_PAWNS_ALL[BLACK]) / 100.0 << "\n";
        cout << "|       none:                     " << setw(10) << static_cast<double>(SCORE_DEBUG.NO_PAWNS[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.NO_PAWNS[BLACK]) / 100.0 << "\n";
        //        cout << "|       pinned:                   " << setw(10) << static_cast<double> (SCORE_DEBUG.PAWN_PINNED[WHITE]) / 100.0 <<
        //             setw(10) << static_cast<double> (SCORE_DEBUG.PAWN_PINNED[BLACK]) / 100.0 << "\n";
        cout << HEADER;
        cout << "|Knight:           " << setw(10) << static_cast<double>(Tresult.knights[WHITE] - Tresult.knights[BLACK]) / 100.0 << setw(15)
             << static_cast<double>(Tresult.knights[WHITE]) / 100.0 << setw(10) << static_cast<double>(Tresult.knights[BLACK]) / 100.0 << "\n";
        ...
        cout << "|       trapped:                  " << setw(10) << static_cast<double>(SCORE_DEBUG.KNIGHT_TRAPPED[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.KNIGHT_TRAPPED[BLACK]) / 100.0 << "\n";
        cout << "|       mobility:                 " << setw(10) << static_cast<double>(SCORE_DEBUG.MOB_KNIGHT[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.MOB_KNIGHT[BLACK]) / 100.0 << "\n";
        cout << "|       pinned:                   " << setw(10) << static_cast<double>(SCORE_DEBUG.KNIGHT_PINNED[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.KNIGHT_PINNED[BLACK]) / 100.0 << "\n";
        cout << HEADER;
        cout << "|Bishop:           " << setw(10) << static_cast<double>(Tresult.bishop[WHITE] - Tresult.bishop[BLACK]) / 100.0 << setw(15)
             << static_cast<double>(Tresult.bishop[WHITE]) / 100.0 << setw(10) << static_cast<double>(Tresult.bishop[BLACK]) / 100.0 << "\n";
        cout << "|       bad:                      " << setw(10) << static_cast<double>(SCORE_DEBUG.BAD_BISHOP[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.BAD_BISHOP[BLACK]) / 100.0 << "\n";
        cout << "|       mobility:                 " << setw(10) << static_cast<double>(SCORE_DEBUG.MOB_BISHOP[WHITE]) / 100.0 << setw(10)
        ...
        cout << "|       bonus 2 bishops:          " << setw(10) << static_cast<double>(SCORE_DEBUG.BONUS2BISHOP[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.BONUS2BISHOP[BLACK]) / 100.0 << "\n";
        cout << "|       pinned:                   " << setw(10) << static_cast<double>(SCORE_DEBUG.BISHOP_PINNED[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.BISHOP_PINNED[BLACK]) / 100.0 << "\n";
        cout << HEADER;
        cout << "|Rook:             " << setw(10) << static_cast<double>(Tresult.rooks[WHITE] - Tresult.rooks[BLACK]) / 100.0 << setw(15)
             << static_cast<double>(Tresult.rooks[WHITE]) / 100.0 << setw(10) << static_cast<double>(Tresult.rooks[BLACK]) / 100.0 << "\n";
        cout << "|       7th:                      " << setw(10) << static_cast<double>(SCORE_DEBUG.ROOK_7TH_RANK[WHITE]) / 100.0 << setw(10)
        ...
        cout << "|       mobility:                 " << setw(10) << static_cast<double>(SCORE_DEBUG.MOB_ROOK[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.MOB_ROOK[BLACK]) / 100.0 << "\n";
        ...
        cout << "|       pinned:                   " << setw(10) << static_cast<double>(SCORE_DEBUG.ROOK_PINNED[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.ROOK_PINNED[BLACK]) / 100.0 << "\n";
        cout << HEADER;
        cout << "|Queen:            " << setw(10) << static_cast<double>(Tresult.queens[WHITE] - Tresult.queens[BLACK]) / 100.0 << setw(15)
             << static_cast<double>(Tresult.queens[WHITE]) / 100.0 << setw(10) << static_cast<double>(Tresult.queens[BLACK]) / 100.0 << "\n";
        cout << "|       mobility:                 " << setw(10) << static_cast<double>(SCORE_DEBUG.MOB_QUEEN[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.MOB_QUEEN[BLACK]) / 100.0 << "\n";
        cout << "|       bishop on queen:          " << setw(10) << static_cast<double>(SCORE_DEBUG.BISHOP_ON_QUEEN[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.BISHOP_ON_QUEEN[BLACK]) / 100.0 << "\n";
        cout << "|       pinned:                   " << setw(10) << static_cast<double>(SCORE_DEBUG.QUEEN_PINNED[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.QUEEN_PINNED[BLACK]) / 100.0 << "\n";
        cout << HEADER;
        cout << "|King:             " << setw(10) << static_cast<double>(Tresult.kings[WHITE] - Tresult.kings[BLACK]) / 100.0 << setw(15)
             << static_cast<double>(Tresult.kings[WHITE]) / 100.0 << setw(10) << static_cast<double>(Tresult.kings[BLACK]) / 100.0 << "\n";
        cout << "|       distance:                 " << setw(10) << static_cast<double>(SCORE_DEBUG.DISTANCE_KING[WHITE]) / 100.0 << setw(10)
             << static_cast<double>(SCORE_DEBUG.DISTANCE_KING[BLACK]) / 100.0 << "\n";

        cout << "|       pawn near:                " << setw(10) << static_cast<double>(SCORE_DEBUG.PAWN_NEAR_KING[WHITE]) / 100.0 << setw(10)
