...
pair<int, int> Eval::evaluateBishop(const _Tchessboard &chessboard, const u64 enemies) {
  ...
  for (; bishop; RESET_LSB(bishop)) {
    ...
    //                   result[MG] += p;
    //                   result[EG] += p;
    //                   if (!(chessboard[KNIGHT_BLACK + xside]) && !(chessboard[BISHOP_BLACK + xside] &
    //                   board::colors(o))) {
    //                       result[MG] += p;
    //                       result[EG] += p;
    //                   }
...
short Eval::getScore(const _Tchessboard &chessboard, const u64 key, const uchar side, const int alpha, const int beta,
  ...
  const int result_mg = (lazyscore_black + tResult[MG].pawns[BLACK] +  // attack_king_black[MG] +
  ...
  int phase =
      ((MAX_VALUE_TAPERED - (lazyscore_white + lazyscore_black)) * 256 + (MAX_VALUE_TAPERED / 2)) / MAX_VALUE_TAPERED;
  ...
  if (trace) {
    ...
    cout << setprecision(2) << "Material:         " << setw(10) << (float)(lazyscore_white - lazyscore_black) / 100.0
         << setw(15) << (float)(lazyscore_white) / 10000.0 << setw(10) << (float)(lazyscore_black) / 10000.0 << endl;
    ...
    cout << endl << "\tBISHOP\t|";
    p2((tResult[MG].bishop[WHITE] - tResult[MG].bishop[BLACK]), (tResult[EG].bishop[WHITE] - tResult[EG].bishop[BLACK]),
       phase);
    ...
    cout << endl << "\tQUEEN\t|";
    p2((tResult[MG].queens[WHITE] - tResult[MG].queens[BLACK]), (tResult[EG].queens[WHITE] - tResult[EG].queens[BLACK]),
       phase);
