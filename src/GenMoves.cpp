...
bool GenMoves::allowCastleBlackQueen(const u64 allpieces) const {
    return POW2_59 & chessboard[KING_BLACK] && rightCastle & RIGHT_QUEEN_CASTLE_BLACK_MASK && !(allpieces & 0x7000000000000000ULL) &&
           chessboard[ROOK_BLACK] & POW2_63 && !board::isAttacked<BLACK>(59, allpieces, chessboard) &&
           !board::isAttacked<BLACK>(60, allpieces, chessboard) && !board::isAttacked<BLACK>(61, allpieces, chessboard);
}

bool GenMoves::allowCastleWhiteQueen(const u64 allpieces) const {
    return POW2_3 & chessboard[KING_WHITE] && !(allpieces & 0x70ULL) && rightCastle & RIGHT_QUEEN_CASTLE_WHITE_MASK &&
           chessboard[ROOK_WHITE] & POW2_7 && !board::isAttacked<WHITE>(3, allpieces, chessboard) &&
           !board::isAttacked<WHITE>(4, allpieces, chessboard) && !board::isAttacked<WHITE>(5, allpieces, chessboard);
}

bool GenMoves::allowCastleBlackKing(const u64 allpieces) const {
    return POW2_59 & chessboard[KING_BLACK] && rightCastle & RIGHT_KING_CASTLE_BLACK_MASK && !(allpieces & 0x600000000000000ULL) &&
           chessboard[ROOK_BLACK] & POW2_56 && !board::isAttacked<BLACK>(57, allpieces, chessboard) &&
           !board::isAttacked<BLACK>(58, allpieces, chessboard) && !board::isAttacked<BLACK>(59, allpieces, chessboard);
}

bool GenMoves::allowCastleWhiteKing(const u64 allpieces) const {
    return POW2_3 & chessboard[KING_WHITE] && !(allpieces & 0x6ULL) && rightCastle & RIGHT_KING_CASTLE_WHITE_MASK &&
           chessboard[ROOK_WHITE] & POW2_0 && !board::isAttacked<WHITE>(1, allpieces, chessboard) &&
           !board::isAttacked<WHITE>(2, allpieces, chessboard) && !board::isAttacked<WHITE>(3, allpieces, chessboard);
}
...
void GenMoves::unPerformCastle(const uchar side, const uchar type) {
    ...
    if (side == WHITE) {
        if (type & KING_SIDE_CASTLE_MOVE_MASK) {
            ...
            if (startPosWhiteKing != 1) chessboard[KING_WHITE] = (chessboard[KING_WHITE] | POW2(startPosWhiteKing)) & NOTPOW2_1;
            if (startPosWhiteRookKingSide != F1) chessboard[ROOK_WHITE] = (chessboard[ROOK_WHITE] | POW2(startPosWhiteRookKingSide)) & NOTPOW2_2;
        } else {
            assert(board::getPieceAt(side, POW2_5, chessboard) == KING_WHITE);
            assert(board::getPieceAt(side, POW2_4, chessboard) == ROOK_WHITE);
            if (startPosWhiteKing != C1) chessboard[KING_WHITE] = (chessboard[KING_WHITE] | POW2(startPosWhiteKing)) & NOTPOW2_5;
            if (startPosWhiteRookQueenSide != D1) chessboard[ROOK_WHITE] = (chessboard[ROOK_WHITE] | POW2(startPosWhiteRookQueenSide)) & NOTPOW2_4;
        }
        assert(chessboard[KING_WHITE] & POW2(startPosWhiteKing));
    } else {
        if (type & KING_SIDE_CASTLE_MOVE_MASK) {
            ...
            if (startPosBlackKing != G8) chessboard[KING_BLACK] = (chessboard[KING_BLACK] | POW2(startPosBlackKing)) & NOTPOW2_57;
            if (startPosBlackRookKingSide != F8) chessboard[ROOK_BLACK] = (chessboard[ROOK_BLACK] | POW2(startPosBlackRookKingSide)) & NOTPOW2_58;
        } else {
            assert(board::getPieceAt(side, POW2_61, chessboard) == KING_BLACK);
            assert(board::getPieceAt(side, POW2_60, chessboard) == ROOK_BLACK);
            if (startPosBlackKing != C8) chessboard[KING_BLACK] = (chessboard[KING_BLACK] | POW2(startPosBlackKing)) & NOTPOW2_61;
            if (startPosBlackRookQueenSide != D8) chessboard[ROOK_BLACK] = (chessboard[ROOK_BLACK] | POW2(startPosBlackRookQueenSide)) & NOTPOW2_60;
        }
        assert(chessboard[KING_BLACK] & POW2(startPosBlackKing));
    }
}
        ...
        if (move->capturedPiece != SQUARE_EMPTY) {
            if (((move->type & 0x3) != ENPASSANT_MOVE_MASK)) chessboard[move->capturedPiece] |= POW2(move->to);
                ...
                if (move->side) chessboard[move->capturedPiece] |= POW2(move->to - 8);
                    ...
                    chessboard[move->capturedPiece] |= POW2(move->to + 8);
            }
        }
...
bool GenMoves::makemove(const _Tmove *move, const bool rep) {
    ...
    if (!(move->type & 0xc)) { // no castle
        ...
        if (POW2(move->to) & 0xff000000000000ffULL) {
            ...
            else if (move->to == startPosWhiteRookQueenSide)
                rightCastle &= 0xdf;
            else if (move->to == startPosBlackRookKingSide)
                rightCastle &= 0xbf;
            else if (move->to == startPosBlackRookQueenSide)
                rightCastle &= 0x7f;
        }
        if (move->pieceFrom & 0xb) {
            ...
            case KING_BLACK:
                ...
                break;
            case ROOK_WHITE:
                if (move->from == startPosWhiteRookKingSide) rightCastle &= 0xef;
                else if (move->from == startPosWhiteRookQueenSide)
                    rightCastle &= 0xdf;
                break;
            case ROOK_BLACK:
                if (move->from == startPosBlackRookKingSide) rightCastle &= 0xbf;
        ...
        if (move->side == WHITE) rightCastle &= 0xcf;
        else
            rightCastle &= 0x3f;
    }
    ...
    if (rep) {
        if (move->capturedPiece != SQUARE_EMPTY || move->pieceFrom == WHITE || move->pieceFrom == BLACK || move->type & 0xc) {
            pushStackMove(0);
        }
        pushStackMove(chessboard[ZOBRISTKEY_IDX]);
    }
    if ((forceCheck || !perftMode) &&
        ((move->side == WHITE && board::inCheck1<WHITE>(chessboard)) || (move->side == BLACK && board::inCheck1<BLACK>(chessboard)))) {
        return false;
    }
    return true;
}
...
int GenMoves::getMoveFromSan(const string &fenStr, _Tmove *move) {
    ...
    movesCount++;
    if ((((MATCH_QUEENSIDE_WHITE.find(fenStr) != string::npos && allowQueenSideWhite(board::getBitmap(chessboard))) ||
          (MATCH_KINGSIDE_WHITE.find(fenStr) != string::npos && allowKingSideWhite(board::getBitmap(chessboard))))) ||
        (((MATCH_QUEENSIDE_BLACK.find(fenStr) != string::npos && allowQueenSideBlack(board::getBitmap(chessboard))) ||
          (MATCH_KINGSIDE_BLACK.find(fenStr) != string::npos && allowKingSideBlack(board::getBitmap(chessboard)))))) {
    ...
    if (fenStr.length() == 4) {
        ...
        if (pieceFrom == PAWN_WHITE || pieceFrom == PAWN_BLACK) {
            if (FILE_AT[from] != FILE_AT[to] &&
                (X(move->side) ? board::getPieceAt<WHITE>(POW2(to), chessboard) : board::getPieceAt<BLACK>(POW2(to), chessboard)) == SQUARE_EMPTY) {
...
void GenMoves::writeRandomFen(const vector<int> pieces) {
    while (1) {
        ...
        if (bitCount(check) == (2 + static_cast<int>(pieces.size())) && !board::inCheck1<WHITE>(chessboard) && !board::inCheck1<BLACK>(chessboard) &&
...
_Tmove *GenMoves::swap(_TmoveP *list, const int i, const int j) {
...
bool GenMoves::generatePuzzle(const string type) {
    ...
    for (unsigned k = 0; k < TOT; k++) {
        uchar side = WHITE;
        pieces.clear();
        ASSERT_RELEASE(toupper(type.at(0)) == 'K');
...
bool GenMoves::verifyMove(const _Tmove *move) {
    ...
    if (!(move->type & 0xc)) { // no castle
        ...
        if (move->capturedPiece != SQUARE_EMPTY) {
            ...
            const bool cap = move->capturedPiece == (PAWN_BLACK + xside) || move->capturedPiece == (KNIGHT_BLACK + xside) ||
