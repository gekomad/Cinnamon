...
int GenMoves::performRankFileShiftCount(const int position, const u64 allpieces) {
    ...
    count += q ? bits.MASK_BIT_SET_NOBOUND_COUNT[position][Bits::BITScanForward(q)]
               : MASK_BIT_SET_COUNT_VERT_UPPER[position];
    ///RANK
    q = allpieces & MASK_BIT_UNSET_RIGHT[position];
    count += q ? bits.MASK_BIT_SET_NOBOUND_COUNT[position][Bits::BITScanForward(q)]
               : MASK_BIT_SET_COUNT_ORIZ_LEFT[position];
...
int GenMoves::performDiagShiftCount(const int position, const u64 allpieces) {
    ...
    u64 q = allpieces & MASK_BIT_UNSET_LEFT_UP[position];
    int count = q ? bits.MASK_BIT_SET_NOBOUND_COUNT[position][Bits::BITScanReverse(q)]
                  : MASK_BIT_SET_COUNT_LEFT_LOWER[position];
    q = allpieces & MASK_BIT_UNSET_LEFT_DOWN[position];
    count += q ? bits.MASK_BIT_SET_NOBOUND_COUNT[position][Bits::BITScanForward(q)]
               : MASK_BIT_SET_COUNT_LEFT_UPPER[position];
    ///RIGHT
    q = allpieces & MASK_BIT_UNSET_RIGHT_UP[position];
    count += q ? bits.MASK_BIT_SET_NOBOUND_COUNT[position][Bits::BITScanReverse(q)]
               : MASK_BIT_SET_COUNT_RIGHT_LOWER[position];
    q = allpieces & MASK_BIT_UNSET_RIGHT_DOWN[position];
    count += q ? bits.MASK_BIT_SET_NOBOUND_COUNT[position][Bits::BITScanForward(q)]
               : MASK_BIT_SET_COUNT_RIGHT_UPPER[position];
    return count;
}
...
int GenMoves::getMobilityPawns(const int side, const int ep, const u64 ped_friends, const u64 enemies,
    ...
    ASSERT_RANGE(side, 0, 1);
    return ep == NO_ENPASSANT ? 0 : Bits::bitCount(ENPASSANT_MASK[side ^ 1][ep] & chessboard[side]) + side == WHITE ?
                                    Bits::bitCount((ped_friends << 8) & xallpieces) + Bits::bitCount(
                                            ((((ped_friends & TABJUMPPAWN) << 8) & xallpieces) << 8) & xallpieces) +
                                    Bits::bitCount((chessboard[side] << 7) & TABCAPTUREPAWN_LEFT & enemies) +
                                    Bits::bitCount((chessboard[side] << 9) & TABCAPTUREPAWN_RIGHT & enemies) :
                                    Bits::bitCount((ped_friends >> 8) & xallpieces) + Bits::bitCount(
                                            ((((ped_friends & TABJUMPPAWN) >> 8) & xallpieces) >> 8) & xallpieces) +
                                    Bits::bitCount((chessboard[side] >> 7) & TABCAPTUREPAWN_RIGHT & enemies) +
                                    Bits::bitCount((chessboard[side] >> 9) & TABCAPTUREPAWN_LEFT & enemies);
}
...
int GenMoves::getMobilityQueen(const int position, const u64 enemies, const u64 friends) {
    ...
    return performRankFileCaptureCount(position, enemies, enemies | friends) +
           Bits::bitCount(enemies & performDiagCaptureCount(position, enemies | friends)) +
...
int GenMoves::getMobilityCastle(const int side, const u64 allpieces) {
    ...
    if (side == WHITE) {
        if (POW2_3 & chessboard[KING_WHITE] && !(allpieces & 0x6ULL) &&
            ...
            !attackSquare<WHITE>(1, allpieces) && !attackSquare<WHITE>(2, allpieces) &&
            !attackSquare<WHITE>(3, allpieces)) {
            count++;
        }
        if (POW2_3 & chessboard[KING_WHITE] && !(allpieces & 0x70ULL) &&
            ...
            !attackSquare<WHITE>(3, allpieces) && !attackSquare<WHITE>(4, allpieces) &&
            !attackSquare<WHITE>(5, allpieces)) {
            count++;
        }
    } else {
        if (POW2_59 & chessboard[KING_BLACK] && chessboard[RIGHT_CASTLE_IDX] & RIGHT_KING_CASTLE_BLACK_MASK &&
            ...
            !attackSquare<BLACK>(57, allpieces) && !attackSquare<BLACK>(58, allpieces) &&
            !attackSquare<BLACK>(59, allpieces)) {
            count++;
        }
        if (POW2_59 & chessboard[KING_BLACK] && chessboard[RIGHT_CASTLE_IDX] & RIGHT_QUEEN_CASTLE_BLACK_MASK &&
            ...
            !attackSquare<BLACK>(59, allpieces) && !attackSquare<BLACK>(60, allpieces) &&
            !attackSquare<BLACK>(61, allpieces)) {
            count++;
        }
    }
    return count;
