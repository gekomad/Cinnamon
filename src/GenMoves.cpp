...
template < uchar type > bool GenMoves::pushmove ( const int from, const int to, const int side, int promotionPiece, int pieceFrom ) {
  ...
  bool
    res = false;
  if ( ( ( type & 0b00000011 ) != ENPASSANT_MOVE_MASK ) && !( type & 0b00001100 ) ) {
    piece_captured = side ? getPieceAt < BLACK > ( POW2[to] ) : getPieceAt < WHITE > ( POW2[to] );

    if ( piece_captured == KING_BLACK + ( side ^ 1 ) )
      res = true;
  }
  else if ( !( type & 0b00001100 ) )	//no castle
    piece_captured = side ^ 1;

  if ( !( type & 0b00001100 ) && ( forceCheck || perftMode ) ) {	//no castle
  ...
  else if ( type & 0b00001100 ) {	//castle
    ASSERT ( rightCastle );
    mos->score = 100;		//80
  }
  mos->used = false;
...
GenMoves::takeback ( _Tmove * move, const u64 oldkey, bool rep ) {
  ...
  rightCastle = move->type & 0b11110000;
  if ( ( move->type & 0b00000011 ) == STANDARD_MOVE_MASK || ( move->type & 0b00000011 ) == ENPASSANT_MOVE_MASK ) {
    posTo = move->to;
    posFrom = move->from;
    ...
    if ( movecapture != SQUARE_FREE ) {
      if ( ( ( move->type & 0b00000011 ) != ENPASSANT_MOVE_MASK ) ) {
  ...
  else if ( ( move->type & 0b00000011 ) == PROMOTION_MOVE_MASK ) {
    posTo = move->to;
    posFrom = move->from;
    movecapture = move->capturedPiece;
  ...
  else if ( move->type & 0b00001100 ) {	//castle
...
GenMoves::makemove ( _Tmove * move, bool rep, bool checkInCheck ) {
  ...
  if ( !( move->type & 0b00001100 ) ) {	//no castle
    posTo = move->to;
    posFrom = move->from;
    ASSERT ( posTo >= 0 );
    movecapture = move->capturedPiece;
    ...
    ASSERT ( posTo >= 0 && posTo < 64 );
    pieceFrom = move->pieceFrom;
    if ( ( move->type & 0b00000011 ) == PROMOTION_MOVE_MASK ) {
    ...
    switch ( pieceFrom ) {
    case KING_WHITE:{
      rightCastle &= 0b11001111;
    }
      break;
    case KING_BLACK:{
      rightCastle &= 0b00111111;
    }
      break;

    case ROOK_WHITE:
      if ( posFrom == 0 ) {
	rightCastle &= 0b11101111;
      }
      else if ( posFrom == 7 ) {
	rightCastle &= 0b11011111;
      }
      break;
    case ROOK_BLACK:
      if ( posFrom == 56 ) {
	rightCastle &= 0b10111111;
      }
      else if ( posFrom == 63 ) {
	rightCastle &= 0b01111111;
      }
      break;
      //en passant
    case PAWN_WHITE:
  ...
  if ( rep ) {
    if ( movecapture != SQUARE_FREE || pieceFrom == WHITE || pieceFrom == BLACK || move->type & 0b00001100 )
      pushStackMove ( 0 );
