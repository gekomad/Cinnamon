...
GenMoves::GenMoves() : perftMode(false), listId(-1) {
  ...
  _assert(genList) for (int i = 0; i < MAX_PLY; i++) {
  ...
  _assert(repetitionMap) repetitionMapCount = 0;
  init();
}
...
void GenMoves::setPerft(const bool b) { perftMode = b; }
...
_Tmove *GenMoves::getNextMove(_TmoveP *list, const int depth, const u64 &hash, const int first) {
  ...
  for (int i = first; i < list->size; i++) {
    ...
    if (move.type & 0x3) {
      ...
      if (isKiller(0, move.from, move.to, depth))
        score += 50;
      else if (isKiller(1, move.from, move.to, depth))
        score += 30;
...
bool GenMoves::allowCastleBlackQueen(const u64 allpieces) const {
  return POW2_59 & chessboard[KING_BLACK] && rightCastle & RIGHT_QUEEN_CASTLE_BLACK_MASK &&
         ...
         !board::isAttacked<BLACK>(59, allpieces, chessboard) && !board::isAttacked<BLACK>(60, allpieces, chessboard) &&
...
bool GenMoves::allowCastleBlackKing(const u64 allpieces) const {
  return POW2_59 & chessboard[KING_BLACK] && rightCastle & RIGHT_KING_CASTLE_BLACK_MASK &&
         ...
         !board::isAttacked<BLACK>(57, allpieces, chessboard) && !board::isAttacked<BLACK>(58, allpieces, chessboard) &&
  ...
  if (move->type & 0x1) {
    ...
    if (move->capturedPiece != SQUARE_EMPTY) {
      if (((move->type & 0x3) != ENPASSANT_MOVE_MASK))
        chessboard[move->capturedPiece] |= POW2(move->to);
      else {
...
bool GenMoves::makemove(const _Tmove *move, const bool rep) {
  ...
  if (!(move->type & 0xc)) {  // no castle
    ...
    if (POW2(move->to) & 0xff000000000000ffULL) {
      if (move->to == startPosWhiteRookKingSide)
        rightCastle &= 0xef;
  ...
  if ((forceCheck || !perftMode) && ((move->side == WHITE && board::inCheck1<WHITE>(chessboard)) ||
...
u64 GenMoves::getTotMoves() const { return numMoves + numMovesq; }

void GenMoves::setRepetitionMapCount(const int i) { repetitionMapCount = i; }
  ...
  if (fenStr.length() == 4) {
    ...
    if (pieceFrom == PAWN_WHITE || pieceFrom == PAWN_BLACK) {
      if (FILE_AT[from] != FILE_AT[to] &&
          (X(move->side) ? board::getPieceAt<WHITE>(POW2(to), chessboard)
                         : board::getPieceAt<BLACK>(POW2(to), chessboard)) == SQUARE_EMPTY) {
...
bool GenMoves::generatePuzzle(const string type) {
  ...
  for (unsigned k = 0; k < TOT; k++) {
    uchar side = WHITE;
    pieces.clear();
    _assert(toupper(type.at(0)) == 'K') for (unsigned i = 1; i < type.size(); i++) {
