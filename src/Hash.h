...
class Hash : public Singleton<Hash> {
  ...
  public:
    typedef struct _Thash {
        ...
        _Thash(const u64 zobristKeyR, const short score, const char depth, const uchar from, const uchar to, const uchar flags) {
            key = zobristKeyR;
            data = static_cast<u64>(static_cast<uint16_t>(score));
    ...
    static __attribute__((always_inline)) void SET_AGE(u64 &u, const int v) {
        u |= static_cast<u64>(v) << (16 + 8 + 8 + 8 + 8);
    }
    static __attribute__((always_inline)) uchar GET_DEPTH(const u64 v) {
        return v >> 16;
    }
    static __attribute__((always_inline)) uchar GET_FLAGS(const u64 v) {
        return v >> (16 + 8 + 8 + 8);
    }
    static __attribute__((always_inline)) uchar GET_FROM(const u64 v) {
        return v >> (16 + 8 + 8);
    }
    static __attribute__((always_inline)) uchar GET_TO(const u64 v) {
        return v >> (16 + 8);
    }
    static __attribute__((always_inline)) short GET_SCORE(const u64 v) {
        return v;
    }
    static __attribute__((always_inline)) unsigned short GET_AGE(const u64 v) {
        return v >> (16 + 8 + 8 + 8 + 8);
    }
    static __attribute__((always_inline)) u64 GET_KEY(const _Thash *hash) {
        return hash->key;
    }
    static __attribute__((always_inline)) void INC_AGE(u64 &u) {
        SET_AGE(u, GET_AGE(u) + 1);
    }
    ...
    static void recordHash(const _Thash &toStore, const int age) {
        ...
        else if (GET_FLAGS(toStore.data) == hashfBETA)
            nRecordHashB++;
        else
            nRecordHashE++;
