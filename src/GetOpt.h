...
static const string PERFT_HELP =
    "-perft [-d depth] [-c nCpu] [-h hash size (mb) [-F dump file]] [-Chess960] [-f \"fen position\"]";
 ...
 private:
  ...
  static void perft(int argc, char **argv) {
    ...
    if (useDump && !FileUtil::fileExists(dumpFile) && !perftHashSize) {
      cout << "Error: with '-F' parameter you have to specify either an existing dump file or a hash size (-h)" << endl
           << endl;
      help(argv);
      return;
    }
