...
string ChessBoard::decodeBoardinv(const _Tmove *move, const bool verbose) const {
    ...
    string res = BOARD[from] + cap + BOARD[to];
...
int ChessBoard::loadFen(const string &fen) {
    ...
    iss >> a2;
    a2 += " 1";
    if (String::isNumber(a2)) movesCount = stoi(a2);
    ...
    if (isChess960()) {
        if (startPosWhiteRookKingSide != NO_POSITION) MATCH_KINGSIDE_WHITE += BOARD[startPosWhiteKing] + BOARD[startPosWhiteRookKingSide];
        if (startPosBlackRookQueenSide != NO_POSITION) MATCH_QUEENSIDE_BLACK += BOARD[startPosBlackKing] + BOARD[startPosBlackRookQueenSide];
        if (startPosWhiteRookQueenSide != NO_POSITION) MATCH_QUEENSIDE_WHITE += BOARD[startPosWhiteKing] + BOARD[startPosWhiteRookQueenSide];
        if (startPosBlackRookKingSide != NO_POSITION) MATCH_KINGSIDE_BLACK += BOARD[startPosBlackKing] + BOARD[startPosBlackRookKingSide];
        if (startPosBlackRookQueenSide != NO_POSITION)
            MATCH_QUEENSIDE += MATCH_QUEENSIDE_BLACK + " " + BOARD[startPosBlackKing] + BOARD[startPosBlackRookQueenSide];
        if (startPosWhiteRookQueenSide != NO_POSITION)
            MATCH_QUEENSIDE += MATCH_QUEENSIDE_WHITE + " " + BOARD[startPosWhiteKing] + BOARD[startPosWhiteRookQueenSide];
    } else {
...
void ChessBoard::display() const {
    ...
    cout << "ep:\t\t\t" << (enPassant == NO_ENPASSANT ? "" : (sideToMove ? BOARD[enPassant + 8] : BOARD[enPassant - 8])) << endl;
