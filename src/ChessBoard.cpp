...
void ChessBoard::display() const {
  ...
  cout << "ep:\t\t\t" << (enPassant == NO_ENPASSANT ? "" : (sideToMove ? BOARD[enPassant + 8] : BOARD[enPassant - 8]))
       << endl;
...
void ChessBoard::print(const _Tmove *move) { cout << decodeBoardinv(move, move->side) << " " << flush; }

string ChessBoard::decodeBoardinv(const _Tmove *move, const uchar side, const bool verbose) {
