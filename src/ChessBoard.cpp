...
ChessBoard::ChessBoard() {
  ...
  if ((sideToMove = loadFen(fenString)) == 2) {
    fatal("Bad FEN position format ", fenString) std::exit(1);
  }
}
...
const string ChessBoard::getFen() const { return fenString; }

int ChessBoard::loadFen() { return loadFen(fenString); }
...
string ChessBoard::boardToFen() const {
  ...
  if (enPassant == NO_ENPASSANT) {
    ...
    fen.append(" ");
    sideToMove ? fen.append(BOARD[enPassant + 8])
               : fen.append(BOARD[enPassant - 8]);
  }
  fen.append(" 0 ");
...
void ChessBoard::display() const {
  ...
  for (int t = 0; t <= 63; t++) {
    ...
    x = (x = (x = FEN_PIECE[board::getPieceAt<WHITE>(POW2(63 - t),
                                                     chessboard)]) != '-'
                 ? x
                 : FEN_PIECE[board::getPieceAt<BLACK>(POW2(63 - t),
                                                      chessboard)]) == '-'
            ? ' '
            : x;
    x != ' ' ? cout << x : POW2(t) & WHITE_SQUARES ? cout << " " : cout << "·";

    if ((t + 1) % 8 == 0) cout << " ║ " << 8 - RANK_AT[t];
    else cout << " │ ";
  }
  cout << endl << "   ╚═══╧═══╧═══╧═══╧═══╧═══╧═══╧═══╝" << endl;
  ...
  cout << "castle:\t\t";
  if (rightCastle & RIGHT_KING_CASTLE_WHITE_MASK)
    cout << whiteRookKingSideCastle;
  if (rightCastle & RIGHT_QUEEN_CASTLE_WHITE_MASK)
    cout << whiteRookQueenSideCastle;
  if (rightCastle & RIGHT_KING_CASTLE_BLACK_MASK)
    cout << blackRookKingSideCastle;
  if (rightCastle & RIGHT_QUEEN_CASTLE_BLACK_MASK)
    cout << blackRookQueenSideCastle;
  cout << endl;

  cout << "ep:\t\t\t"
       << (enPassant == NO_ENPASSANT
               ? ""
               : (sideToMove ? BOARD[enPassant + 8] : BOARD[enPassant - 8]))
       << endl;
  cout << "Chess960:\t" << (chess960 ? "true" : "false") << endl;
  DEBUG(cout << "zobristKey:\t0x" << hex << chessboard[ZOBRISTKEY_IDX] << "ull"
             << dec << endl)
  cout << endl;
}
...
string ChessBoard::decodeBoardinv(const _Tmove *move, const bool verbose) const {
  ...
  if (type & QUEEN_SIDE_CASTLE_MOVE_MASK && side == WHITE) {
    return isChess960()
               ? BOARD[startPosWhiteKing] + BOARD[startPosWhiteRookQueenSide]
               : "e1c1";
  }
  if (type & KING_SIDE_CASTLE_MOVE_MASK && side == WHITE) {
    return isChess960()
               ? BOARD[startPosWhiteKing] + BOARD[startPosWhiteRookKingSide]
               : "e1g1";
  }
  if (type & QUEEN_SIDE_CASTLE_MOVE_MASK && side == BLACK) {
    return isChess960()
               ? BOARD[startPosBlackKing] + BOARD[startPosBlackRookQueenSide]
               : "e8c8";
  }
  if (type & KING_SIDE_CASTLE_MOVE_MASK && side == BLACK) {
    return isChess960()
               ? BOARD[startPosBlackKing] + BOARD[startPosBlackRookKingSide]
               : "e8g8";
  }
...
int ChessBoard::loadFen(const string &fen) {
  ...
  auto blackRookKingSide = [&](const char c) {
    startPosBlackRookKingSide =
        BITScanForward(chessboard[ROOK_BLACK] & 0xff00000000000000ULL);
  ...
  auto whiteRookQueenSide = [&](const char c) {
    startPosWhiteRookQueenSide =
        BITScanReverse(chessboard[ROOK_WHITE] & 0xffULL);
  ...
  auto blackRookQueenSide = [&](const char c) {
    startPosBlackRookQueenSide =
        BITScanReverse(chessboard[ROOK_BLACK] & 0xff00000000000000ULL);
  ...
  if (isChess960()) {
    if (startPosWhiteRookKingSide != NO_POSITION)
      MATCH_KINGSIDE_WHITE +=
          BOARD[startPosWhiteKing] + BOARD[startPosWhiteRookKingSide];
    if (startPosBlackRookQueenSide != NO_POSITION)
      MATCH_QUEENSIDE_BLACK +=
          BOARD[startPosBlackKing] + BOARD[startPosBlackRookQueenSide];
    if (startPosWhiteRookQueenSide != NO_POSITION)
      MATCH_QUEENSIDE_WHITE +=
          BOARD[startPosWhiteKing] + BOARD[startPosWhiteRookQueenSide];
    if (startPosBlackRookKingSide != NO_POSITION)
      MATCH_KINGSIDE_BLACK +=
          BOARD[startPosBlackKing] + BOARD[startPosBlackRookKingSide];
    if (startPosBlackRookQueenSide != NO_POSITION)
      MATCH_QUEENSIDE += MATCH_QUEENSIDE_BLACK + " " +
                         BOARD[startPosBlackKing] +
                         BOARD[startPosBlackRookQueenSide];
    if (startPosWhiteRookQueenSide != NO_POSITION)
      MATCH_QUEENSIDE += MATCH_QUEENSIDE_WHITE + " " +
                         BOARD[startPosWhiteKing] +
                         BOARD[startPosWhiteRookQueenSide];
  } else {
