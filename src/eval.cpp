...
evaluate_king ( const int side, const u64 queen_nemica, const u64 PAWNS_friends, const int pos_re, const u64 pezzi_friends, const u64 pezzi_enemies ) {
  ...
  if ( !FINE_APERTURA && ( ( evalNode.open_column[side] & col | evalNode.colonna_semi_aperta[side] & col ) || ( ( col <= 15 && col > 1 )
														&& ( evalNode.open_column[side] & ( col >> 1 )
														     | evalNode.colonna_semi_aperta[side] & ( col >> 1 ) ) )
			   || ( ( col != 128 )
...
#ifdef HASH_MODE
  if ( keyzobrist && use_book && ( ( mat = search_openbook ( keyzobrist, -1 ) ) != -1 ) ) {
#ifdef DEBUG_MODE
