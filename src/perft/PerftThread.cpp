...
//const int PerftThread::N_MUTEX_BUCKET = 4096;
...
void PerftThread::run() {
    ...
    for (int ii = to - 1; ii >= from; ii--) {
        ...
        if (fhash) {
            if (side == WHITE) {
                if (smp) {
                    n_perft = search<WHITE, true, true>(tPerftRes->depth - 1);
                } else {//smp == false
                    n_perft = search<WHITE, true, false>(tPerftRes->depth - 1);
                }
            } else {
                if (smp) {
                    n_perft = search<BLACK, true, true>(tPerftRes->depth - 1);
                } else {//smp == false
                    n_perft = search<BLACK, true, false>(tPerftRes->depth - 1);
                }
            }
        } else {//no hash
            if (side == WHITE) {
                n_perft = search<WHITE, false, false>(tPerftRes->depth - 1);
            } else {
                n_perft = search<BLACK, false, false>(tPerftRes->depth - 1);
            }
        }
