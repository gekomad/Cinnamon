...
int GTB::getCache() const { return cacheSize; }
...
string GTB::getPath() const { return path; }
...
int GTB::getDtmWdl(const int stm, const int doPrint, const _Tchessboard &chessboard, unsigned *pliestomate,
  ...
  if (dtm) {
    int tb_available = tb_probe_soft(stm, tb_NOSQUARE, gtbChessboard.tb_castling, gtbChessboard.ws, gtbChessboard.bs,
                                     gtbChessboard.wp, gtbChessboard.bp, &info, pliestomate);
    if (!tb_available)
      tb_available = tb_probe_hard(stm, tb_NOSQUARE, gtbChessboard.tb_castling, gtbChessboard.ws, gtbChessboard.bs,
                                   gtbChessboard.wp, gtbChessboard.bp, &info, pliestomate);
    ...
    if (!tb_available)
      tb_available = tb_probe_WDL_hard(stm, tb_NOSQUARE, gtbChessboard.tb_castling, gtbChessboard.ws, gtbChessboard.bs,
                                       gtbChessboard.wp, gtbChessboard.bp, &info);
    ...
    if (tb_available) {
      if (doPrint != 0) {
        ...
        switch (info) {
          case tb_WMATE:
            if (stm1 == tb_WHITE_TO_MOVE)
              cout << "Loss" << endl;
            else
              cout << "Win" << endl;
            break;
          case tb_BMATE:
            if (stm1 == tb_BLACK_TO_MOVE)
              cout << "Loss" << endl;
            else
              cout << "Win" << endl;
            break;
