...
static Byte MatchFinder_GetIndexByte(CMatchFinder *p, Int32 idx) {
    return p->buffer[idx];
}
...
void MatchFinder_ReadIfRequired(CMatchFinder *p) {
    ...
    if (p->keepSizeAfter >= p->streamPos - p->pos) MatchFinder_ReadBlock(p);
}
...
static CLzRef *AllocRefs(UInt32 num, ISzAlloc *alloc) {
    ...
    if (sizeInBytes / sizeof(CLzRef) != num) return 0;
    return (CLzRef *)alloc->Alloc(alloc, sizeInBytes);
}

int MatchFinder_Create(CMatchFinder *p, UInt32 historySize, UInt32 keepAddBufferBefore, UInt32 matchMaxLen, UInt32 keepAddBufferAfter,
                       ISzAlloc *alloc) {
    UInt32 sizeReserv;
    ...
    if (historySize > ((UInt32)2 << 30)) sizeReserv = historySize >> 2;
    ...
    if (LzInWindow_Create(p, sizeReserv, alloc)) {
...

        {
            ...
            if (p->hash != 0 && prevSize == newSize) return 1;
            MatchFinder_FreeThisClassMemory(p, alloc);
...
static UInt32 *Hc_GetMatchesSpec(UInt32 lenLimit, UInt32 curMatch, UInt32 pos, const Byte *cur, CLzRef *son, UInt32 _cyclicBufferPos,
                                 UInt32 _cyclicBufferSize, UInt32 cutValue, UInt32 *distances, UInt32 maxLen) {
...
UInt32 *GetMatchesSpec1(UInt32 lenLimit, UInt32 curMatch, UInt32 pos, const Byte *cur, CLzRef *son, UInt32 _cyclicBufferPos, UInt32 _cyclicBufferSize,
...
static void SkipMatchesSpec(UInt32 lenLimit, UInt32 curMatch, UInt32 pos, const Byte *cur, CLzRef *son, UInt32 _cyclicBufferPos,
                            UInt32 _cyclicBufferSize, UInt32 cutValue) {
    CLzRef *ptr0 = son + (_cyclicBufferPos << 1) + 1;
    CLzRef *ptr1 = son + (_cyclicBufferPos << 1);
    UInt32 len0 = 0, len1 = 0;
    for (;;) {
...
static UInt32 Bt4_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances) {
    ...
    p->hash[hash2Value] = p->hash[kFix3HashSize + hash3Value] = p->hash[kFix4HashSize + hashValue] = p->pos;
