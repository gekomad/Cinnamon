...
StopCompilingDueBUG
    ...
    static const Byte kLiteralNextStates[kNumStates * 2] = {0, 0, 0, 0, 1, 2, 3, 4,  5,  6,  4,  5,
  ...
  do {
    ...
    IF_BIT_0(prob) {
      ...
      if (state >= kNumStates) {
        ...
        rep0 = distance + 1;
        if (checkDicSize == 0) {
          if (distance >= processedPos) return SZ_ERROR_DATA;
      ...
      if (limit == dicPos) return SZ_ERROR_DATA;
      {
...
static void MY_FAST_CALL LzmaDec_WriteRem(CLzmaDec *p, SizeT limit) {
  if (p->remainLen != 0 && p->remainLen < kMatchSpecLenStart) {
    ...
    if (limit - dicPos < len) len = (unsigned)(limit - dicPos);
...
static int MY_FAST_CALL LzmaDec_DecodeReal2(CLzmaDec *p, SizeT limit, const Byte *bufLimit) {
  do {
    ...
    if (p->checkDicSize == 0) {
      ...
      if (limit - p->dicPos > rem) limit2 = p->dicPos + rem;
    }
    RINOK(LzmaDec_DecodeReal(p, limit2, bufLimit));
    if (p->processedPos >= p->prop.dicSize) p->checkDicSize = p->prop.dicSize;
...

  {
    ...
    IF_BIT_0_CHECK(prob) {
      ...
      if (p->checkDicSize != 0 || p->processedPos != 0)
        prob += (LZMA_LIT_SIZE * ((unsigned)/*MAB casts */
...
SRes LzmaDec_DecodeToDic(CLzmaDec *p, SizeT dicLimit, const Byte *src, SizeT *srcLen, ELzmaFinishMode finishMode,
                         ELzmaStatus *status) {
  SizeT inSize = *srcLen;
  (*srcLen) = 0;
...
SRes LzmaDecode(Byte *dest, SizeT *destLen, const Byte *src, SizeT *srcLen, const Byte *propData, unsigned propSize,
