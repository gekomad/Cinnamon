...
#define GetPosSlot(pos, res)                                                                                                                         \
    {                                                                                                                                                \
        if (pos < kNumFullDistances) res = p->g_FastPos[pos];                                                                                        \
        else                                                                                                                                         \
            BSR2_RET(pos, res);                                                                                                                      \
    }
...
SRes LzmaEnc_SetProps(CLzmaEncHandle pp, const CLzmaEncProps *props2) {
    ...
    if (props.lc > LZMA_LC_MAX || props.lp > LZMA_LP_MAX || props.pb > LZMA_PB_MAX || props.dictSize > (1u << kDicLogSizeMaxCompress) ||
        props.dictSize > (1u << 30)) /*MAB: 1u to silence warnings */
...
static void LzmaEnc_InitPriceTables(UInt32 *ProbPrices) {
    ...
    for (i = (1 << kNumMoveReducingBits) / 2; i < kBitModelTotal; i += (1 << kNumMoveReducingBits)) {
        ...
        ProbPrices[i >> kNumMoveReducingBits] =
            (/*MAB: cast to silence*/ (UInt32)(kNumBitModelTotalBits << kCyclesBits) - /*MAB: cast to silence*/ (UInt32)15 - bitCount);
    }
}
...
static UInt32 GetOptimum(CLzmaEnc *p, UInt32 position, UInt32 *backRes) {
    ...
    if (p->additionalOffset == 0) mainLen = ReadMatchDistances(p, &numPairs_);
    else {
...

    {
        ...
        p->opt[1].price = GET_PRICE_0(p->isMatch[p->state][posState_]) + (!IsCharState(p->state)
    ...
    if (len <= mainLen) {
        ...
        for (;; len++) {
            ...
            if (distance < kNumFullDistances) curAndLenPrice += p->distancesPrices[lenToPosState][distance];
            else {
                UInt32 slot;
    ...
    for (;;) {
        ...
        if (curOpt->prev1IsChar) {
            ...
            if (curOpt->prev2) {
                ...
                if (curOpt->backPrev2 < LZMA_NUM_REPS) state = (UInt32)kRepNextStates[state]; /*MAB casts */
                else
            ...
            state = p->opt[posPrev].state;
        if (posPrev == cur - 1) {
            if (IsShortRep(curOpt)) state = (UInt32)kShortRepNextStates[state]; /*MAB casts */
            else
                state = (UInt32)kLiteralNextStates[state]; /*MAB casts */
        } else {
            UInt32 pos;
            ...
            if (curOpt->prev1IsChar && curOpt->prev2) {
                ...
                pos = curOpt->backPrev;
                if (pos < LZMA_NUM_REPS) state = (UInt32)kRepNextStates[state]; /*MAB casts */
                else
                    state = (UInt32)kMatchNextStates[state]; /*MAB casts */
            }
                ...
                /* if (_maxMode) */
                {
                    ...
                    if (lenTest2 >= 2) {
                        ...
                        UInt32 curAndLenCharPrice =
                            price + p->repLenEnc.prices[posState][lenTest - 2] + GET_PRICE_0(p->isMatch[state2][posStateNext]) +
                            LitEnc_GetPriceMatched(LIT_PROBS(position + lenTest, data[lenTest - 1]), data[lenTest], data2[lenTest], p->ProbPrices);
        ...
        if (newLen >= startLen) {
            ...
            for (lenTest = /*2*/ startLen;; lenTest++) {
                ...
                COptimal *opt__;
                if (curBack < kNumFullDistances) curAndLenPrice__ += p->distancesPrices[lenToPosState][curBack];
                else
                ...
                if (/*_maxMode && */ lenTest == matches[offs]) {
                    ...
                    if (lenTest2 >= 2) {
                        ...
                        UInt32 posStateNext = (position + lenTest) & p->pbMask;
                        UInt32 curAndLenCharPrice =
                            curAndLenPrice__ + GET_PRICE_0(p->isMatch[state2][posStateNext]) +
                            LitEnc_GetPriceMatched(LIT_PROBS(position + lenTest, data[lenTest - 1]), data[lenTest], data2[lenTest], p->ProbPrices);
                        state2 = (UInt32)kLiteralNextStates[state2]; /*MAB casts */
...
static UInt32 GetOptimumFast(CLzmaEnc *p, UInt32 *backRes) {
    ...
    *backRes = (UInt32)-1;
    ...
    if (repLen >= 2 &&
        ((repLen + 1 >= mainLen) || (repLen + 2 >= mainLen && mainDist >= (1 << 9)) || (repLen + 3 >= mainLen && mainDist >= (1 << 15)))) {
        *backRes = repIndex;
        MovePos(p, repLen - 1);
        return repLen;
    }
    ...
    if (p->longestMatchLength >= 2) {
        ...
        if ((p->longestMatchLength >= mainLen && newDistance < mainDist) ||
            (p->longestMatchLength == mainLen + 1 && !ChangePair(mainDist, newDistance)) || (p->longestMatchLength > mainLen + 1) ||
        ...
        for (;;) {
            ...
            if (len == 1 && pos == (UInt32)-1) {
                ...
                if (pos < LZMA_NUM_REPS) {
                    ...
                    if (len == 1) p->state = (UInt32)kShortRepNextStates[p->state]; /*MAB casts */
                    else {
            ...
            if (p->additionalOffset == 0) {
                ...
                if (useLimits) {
                    if (processed + kNumOpts + 300 >= maxUnpackSize || RangeEnc_GetProcessed(&p->rc) + kNumOpts * 2 >= maxPackSize) break;
...
static SRes LzmaEnc_Prepare(CLzmaEncHandle pp, ISeqInStream *inStream, ISeqOutStream *outStream, ISzAlloc *alloc, ISzAlloc *allocBig) {
    CLzmaEnc *p = (CLzmaEnc *)pp;
    p->inStream = inStream;
...
#if defined(USE_UNUSED_CODE)
/*MAB: static added */
static SRes LzmaEnc_PrepareForLzma2(CLzmaEncHandle pp, ISeqInStream *inStream, UInt32 keepWindowSize, ISzAlloc *alloc, ISzAlloc *allocBig) {
    CLzmaEnc *p = (CLzmaEnc *)pp;
    p->inStream = inStream;
...
#if defined(USE_UNUSED_CODE)
/*MAB: static added */
static SRes LzmaEnc_MemPrepare(CLzmaEncHandle pp, const Byte *src, SizeT srcLen, UInt32 keepWindowSize, ISzAlloc *alloc, ISzAlloc *allocBig) {
    CLzmaEnc *p = (CLzmaEnc *)pp;
...
/*MAB: static added */
static SRes LzmaEnc_CodeOneMemBlock(CLzmaEncHandle pp, Bool reInit, Byte *dest, size_t *destLen, UInt32 desiredPackSize, UInt32 *unpackSize) {
    CLzmaEnc *p = (CLzmaEnc *)pp;
    UInt64 nowPos64;
    SRes res;
...
SRes LzmaEnc_Encode(CLzmaEncHandle pp, ISeqOutStream *outStream, ISeqInStream *inStream, ICompressProgress *progress, ISzAlloc *alloc,
                    ISzAlloc *allocBig) {
    CLzmaEnc *p = (CLzmaEnc *)pp;
    SRes res = SZ_OK;
...
SRes LzmaEnc_MemEncode(CLzmaEncHandle pp, Byte *dest, SizeT *destLen, const Byte *src, SizeT srcLen, int writeEndMark, ICompressProgress *progress,
...
SRes LzmaEncode(Byte *dest, SizeT *destLen, const Byte *src, SizeT srcLen, const CLzmaEncProps *props, Byte *propsEncoded, SizeT *propsSize,
