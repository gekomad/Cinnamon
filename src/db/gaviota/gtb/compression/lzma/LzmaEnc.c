...
void LzmaEncProps_Normalize(CLzmaEncProps *p) {
  ...
  if (p->dictSize == 0)
    p->dictSize =
        (UInt32)((level <= 5 ? (1 << (level * 2 + 14)) : (level == 6 ? (1 << 25) : (1 << 26)))); /*MAB casts UInt32 */
  if (p->lc < 0) p->lc = 3;
  if (p->lp < 0) p->lp = 0;
  if (p->pb < 0) p->pb = 2;
...
#define BSR2_RET(pos, res)                        \
  {                                               \
    unsigned long i__;                            \
    _BitScanReverse(&i__, (pos));                 \
    res = (i__ + i__) + ((pos >> (i__ - 1)) & 1); \
  }
...
static void LzmaEnc_FastPosInit(Byte *g_FastPos) {
  ...
  for (slotFast = 2; slotFast < kNumLogBits * 2; slotFast++) {
    ...
    UInt32 j;
    for (j = 0; j < k; j++, c++) g_FastPos[c] = (Byte)slotFast;
  }
}
...
#define BSR2_RET(pos, res)                                                                               \
  {                                                                                                      \
    UInt32 i__ = 6 + ((kNumLogBits - 1) & (0 - (((((UInt32)1 << (kNumLogBits + 6)) - 1) - pos) >> 31))); \
...
static SRes MyRead(void *pp, void *data, size_t *size) {
  ...
  if (p->rem < curSize) curSize = p->rem;
...
SRes LzmaEnc_SetProps(CLzmaEncHandle pp, const CLzmaEncProps *props2) {
  ...
  p->matchFinderCycles = props.mc;
  {
    ...
    if (fb < 5) fb = 5;
    if (fb > LZMA_MATCH_LEN_MAX) fb = LZMA_MATCH_LEN_MAX;
    p->numFastBytes = fb;
  }
...
static void RangeEnc_FlushData(CRangeEnc *p) {
  int i;
  for (i = 0; i < 5; i++) RangeEnc_ShiftLow(p);
}
...
static void LzmaEnc_InitPriceTables(UInt32 *ProbPrices) {
  ...
  for (i = (1 << kNumMoveReducingBits) / 2; i < kBitModelTotal; i += (1 << kNumMoveReducingBits)) {
    ...
    ProbPrices[i >> kNumMoveReducingBits] = (/*MAB: cast to silence*/ (UInt32)(kNumBitModelTotalBits << kCyclesBits) -
                                             /*MAB: cast to silence*/ (UInt32)15 - bitCount);
  }
}
...
static void LenEnc_SetPrices(CLenEnc *p, UInt32 posState, UInt32 numSymbols, UInt32 *prices, UInt32 *ProbPrices) {
  ...
  for (; i < kLenNumLowSymbols + kLenNumMidSymbols; i++) {
    if (i >= numSymbols) return;
    prices[i] =
        b0 + RcTree_GetPrice(p->mid + (posState << kLenNumMidBits), kLenNumMidBits, i - kLenNumLowSymbols, ProbPrices);
  }
  for (; i < numSymbols; i++)
...
static void LenPriceEnc_UpdateTables(CLenPriceEnc *p, UInt32 numPosStates, UInt32 *ProbPrices) {
  UInt32 posState;
  for (posState = 0; posState < numPosStates; posState++) LenPriceEnc_UpdateTable(p, posState, ProbPrices);
}

static void LenEnc_Encode2(CLenPriceEnc *p, CRangeEnc *rc, UInt32 symbol, UInt32 posState, Bool updatePrice,
                           UInt32 *ProbPrices) {
  LenEnc_Encode(&p->p, rc, symbol, posState);
  if (updatePrice)
    if (--p->counters[posState] == 0) LenPriceEnc_UpdateTable(p, posState, ProbPrices);
}
...
static UInt32 ReadMatchDistances(CLzmaEnc *p, UInt32 *numDistancePairsRes) {
  ...
  {
    UInt32 i;
    for (i = 0; i < numPairs; i += 2) printf("%2d %6d   | ", p->matches[i], p->matches[i + 1]);
  }
#endif
  if (numPairs > 0) {
...
static UInt32 GetRepPrice(CLzmaEnc *p, UInt32 repIndex, UInt32 len, UInt32 state, UInt32 posState) {
  return p->repLenEnc.prices[posState][len - LZMA_MATCH_LEN_MIN] + GetPureRepPrice(p, repIndex, state, posState);
}
...
static UInt32 Backward(CLzmaEnc *p, UInt32 *backRes, UInt32 cur) {
  ...
  do {
    if (p->opt[cur].prev1IsChar) {
      MakeAsChar(&p->opt[posMem]) p->opt[posMem].posPrev = posMem - 1;
      if (p->opt[cur].prev2) {
...
#define LIT_PROBS(pos, prevByte) \
  (p->litProbs + ((((pos) & p->lpMask) << p->lc) + (UInt32)((prevByte) >> (8 - p->lc))) * 0x300) /*MAB cast UInt32 */
...
static UInt32 GetOptimum(CLzmaEnc *p, UInt32 position, UInt32 *backRes) {
  ...
  UInt32 matchPrice_, repMatchPrice_, normalMatchPrice;
...

  {
    ...
    p->opt[1].price = GET_PRICE_0(p->isMatch[p->state][posState_]) +
                      (!IsCharState(p->state) ? LitEnc_GetPriceMatched(probs, curByte_, matchByte_, p->ProbPrices)
  ...
  len = lenEnd;
  do p->opt[len--].price = kInfinityPrice;
  while (len >= 2);
  ...
  for (;;) {
    ...
    if (posPrev == cur - 1) {
      ...
      if (pos < LZMA_NUM_REPS) {
        ...
        for (i__ = 1; i__ <= pos; i__++) reps[i__] = prevOpt->backs[i__ - 1];
        for (; i__ < LZMA_NUM_REPS; i__++) reps[i__] = prevOpt->backs[i__];
      } else {
        /*MAB: i changed to i__ to avoid hiding a variable */
        UInt32 i__;
        reps[0] = (pos - LZMA_NUM_REPS);
        for (i__ = 1; i__ < LZMA_NUM_REPS; i__++) reps[i__] = prevOpt->backs[i__ - 1];
      }
    }
    ...
    curAnd1Price = curPrice + GET_PRICE_0(p->isMatch[state][posState]);
    {
      ...
      curAnd1Price += (!IsCharState(state) ? LitEnc_GetPriceMatched(probs, curByte, matchByte, p->ProbPrices)
    ...
    if (!nextIsChar && matchByte != curByte) /* speed optimization */
    {
      ...
      if (lenTest2 >= 2) {
        ...
        UInt32 nextRepMatchPrice =
            curAnd1Price + GET_PRICE_1(p->isMatch[state2][posStateNext]) + GET_PRICE_1(p->isRep[state2]);
      ...
      for (repIndex = 0; repIndex < LZMA_NUM_REPS; repIndex++) {
        ...
        /* if (_maxMode) */
        {
          ...
          if (lenTest2 >= 2) {
            ...
            UInt32 curAndLenCharPrice = price + p->repLenEnc.prices[posState][lenTest - 2] +
            ...
            nextRepMatchPrice =
                curAndLenCharPrice + GET_PRICE_1(p->isMatch[state2][posStateNext]) + GET_PRICE_1(p->isRep[state2]);
      ...
      for (lenTest = /*2*/ startLen;; lenTest++) {
        ...
        if (/*_maxMode && */ lenTest == matches[offs]) {
          ...
          if (lenTest2 >= 2) {
            ...
            UInt32 posStateNext = (position + lenTest) & p->pbMask;
            UInt32 curAndLenCharPrice = curAndLenPrice__ + GET_PRICE_0(p->isMatch[state2][posStateNext]) +
                                        LitEnc_GetPriceMatched(LIT_PROBS(position + lenTest, data[lenTest - 1]),
                                                               data[lenTest], data2[lenTest], p->ProbPrices);
            state2 = (UInt32)kLiteralNextStates[state2]; /*MAB casts */
...
static UInt32 GetOptimumFast(CLzmaEnc *p, UInt32 *backRes) {
  ...
  *backRes = (UInt32)-1;
  ...
  if (repLen >= 2 && ((repLen + 1 >= mainLen) || (repLen + 2 >= mainLen && mainDist >= (1 << 9)) ||
...
static SRes CheckErrors(CLzmaEnc *p) {
  ...
  if (p->result != SZ_OK) p->finished = True;
  return p->result;
}
...
static void FillDistancesPrices(CLzmaEnc *p) {
  ...
  for (j = kStartPosModelIndex; j < kNumFullDistances; j++) {
    ...
    tempPrices[j] = RcTree_ReverseGetPrice(p->posEncoders + base - posSlot - 1, (int)footerBits, j - base,
                                           p->ProbPrices); /*MAB casts */
  }
  ...
  for (lenToPosState = 0; lenToPosState < kNumLenToPosStates; lenToPosState++) {
...

    {
      ...
      /*MAB: i changed to i__ to avoid hiding a variable */
      UInt32 i__;
      for (i__ = 0; i__ < kStartPosModelIndex; i__++) distancesPrices[i__] = posSlotPrices[i__];
      for (; i__ < kNumFullDistances; i__++) distancesPrices[i__] = posSlotPrices[GetPosSlot1(i__)] + tempPrices[i__];
    }
  }
  p->matchPriceCount = 0;
}
        ...
        if (pos < LZMA_NUM_REPS) {
          ...
          if (posSlot >= kStartPosModelIndex) {
            ...
            if (posSlot < kEndPosModelIndex)
              RcTree_ReverseEncode(&p->rc, p->posEncoders + base - posSlot - 1, (int)footerBits,
                                   posReduced); /*MAB casts */
            else {
              RangeEnc_EncodeDirectBits(&p->rc, posReduced >> kNumAlignBits,
                                        (int)footerBits - kNumAlignBits); /*MAB casts */
...
static SRes LzmaEnc_Alloc(CLzmaEnc *p, UInt32 keepWindowSize, ISzAlloc *alloc, ISzAlloc *allocBig) {
  ...
  btMode = (p->matchFinderBase.btMode !=
            0); /*MAB: this line moved inside the compilation ifdef, btMode not used without it */
...

  {
    ...
    if (p->litProbs == 0 || p->saveState.litProbs == 0 || p->lclp != lclp) {
      ...
      p->saveState.litProbs =
          (CLzmaProb *)alloc->Alloc(alloc, (0x300u << lclp) * sizeof(CLzmaProb)); /*MAB 0x300 vs 0x300u*/
...
static void LzmaEnc_Init(CLzmaEnc *p) {
  UInt32 i;
  p->state = 0;
  for (i = 0; i < LZMA_NUM_REPS; i++) p->reps[i] = 0;
...

  {
    ...
    for (i = 0; i < num; i++) p->litProbs[i] = kProbInitValue;
  }

  {
    for (i = 0; i < kNumLenToPosStates; i++) {
      ...
      UInt32 j;
      for (j = 0; j < (1 << kNumPosSlotBits); j++) probs[j] = kProbInitValue;
    }
  }
  {
    for (i = 0; i < kNumFullDistances - kEndPosModelIndex; i++) p->posEncoders[i] = kProbInitValue;
  }

  LenEnc_Init(&p->lenEnc.p);
  LenEnc_Init(&p->repLenEnc.p);

  for (i = 0; i < (1 << kNumAlignBits); i++) p->posAlignEncoder[i] = kProbInitValue;
...
static SRes LzmaEnc_AllocAndInit(CLzmaEnc *p, UInt32 keepWindowSize, ISzAlloc *alloc, ISzAlloc *allocBig) {
  ...
  for (i = 0; i < (UInt32)kDicLogSizeMaxCompress; i++)
    if (p->dictSize <= ((UInt32)1 << i)) break;
  p->distTableSize = i * 2;
...
static SRes LzmaEnc_Prepare(CLzmaEncHandle pp, ISeqInStream *inStream, ISeqOutStream *outStream, ISzAlloc *alloc,
                            ISzAlloc *allocBig) {
  CLzmaEnc *p = (CLzmaEnc *)pp;
  p->inStream = inStream;
...
#if defined(USE_UNUSED_CODE)
/*MAB: static added */
static SRes LzmaEnc_PrepareForLzma2(CLzmaEncHandle pp, ISeqInStream *inStream, UInt32 keepWindowSize, ISzAlloc *alloc,
                                    ISzAlloc *allocBig) {
  CLzmaEnc *p = (CLzmaEnc *)pp;
  p->inStream = inStream;
...
#if defined(USE_UNUSED_CODE)
/*MAB: static added */
static SRes LzmaEnc_MemPrepare(CLzmaEncHandle pp, const Byte *src, SizeT srcLen, UInt32 keepWindowSize, ISzAlloc *alloc,
                               ISzAlloc *allocBig) {
  CLzmaEnc *p = (CLzmaEnc *)pp;
...
#ifdef COMPRESS_MF_MT
  CLzmaEnc *p = (CLzmaEnc *)pp;
  if (p->mtMode) MatchFinderMt_ReleaseStream(&p->matchFinderMt);
#endif
}
...
/*MAB: static added */
static SRes LzmaEnc_CodeOneMemBlock(CLzmaEncHandle pp, Bool reInit, Byte *dest, size_t *destLen, UInt32 desiredPackSize,
                                    UInt32 *unpackSize) {
  CLzmaEnc *p = (CLzmaEnc *)pp;
  UInt64 nowPos64;
  SRes res;
...
SRes LzmaEnc_Encode(CLzmaEncHandle pp, ISeqOutStream *outStream, ISeqInStream *inStream, ICompressProgress *progress,
  ...
  int i = 0;
  for (i = 0; i < 16; i++) allocaDummy[i] = (Byte)i;
#endif
...
SRes LzmaEnc_WriteProperties(CLzmaEncHandle pp, Byte *props, SizeT *size) {
  ...
  *size = LZMA_PROPS_SIZE;
  ...
  for (i = 0; i < 4; i++) props[1 + i] = (Byte)(dictSize >> (8 * i));
  return SZ_OK;
}

SRes LzmaEnc_MemEncode(CLzmaEncHandle pp, Byte *dest, SizeT *destLen, const Byte *src, SizeT srcLen, int writeEndMark,
  ...
  res = LzmaEnc_Encode(pp, &outStream.funcTable, &p->seqBufInStream.funcTable, progress, alloc, allocBig);
...
SRes LzmaEncode(Byte *dest, SizeT *destLen, const Byte *src, SizeT srcLen, const CLzmaEncProps *props,
                Byte *propsEncoded, SizeT *propsSize, int writeEndMark, ICompressProgress *progress, ISzAlloc *alloc,
                ISzAlloc *allocBig) {
  ...
  if (res == SZ_OK) {
    ...
    if (res == SZ_OK) res = LzmaEnc_MemEncode(p, dest, destLen, src, srcLen, writeEndMark, progress, alloc, allocBig);
  }
