...
class TB {
 ...
 public:
  static int probeWdl(const int depth, const uchar side, const int N_PIECE, const int mainDepth, uchar rightCastle,
  ...
  static bool probeRootTB(_Tmove *res, GenMoves &genMoves) {
    ...
    if (GTB::getInstance().isInstalledPieces(tot)) {
      ...
      for (int i = 0; i < genMoves.getListSize(); i++) {
        ...
        BENCH_START("gtbTime")
        const auto res = GTB::getInstance().getDtmWdl(GTB_STM(X(genMoves.sideToMove)), 0, chessboard, &dtz, true,
  ...
  static string probeRootTB1(GenMoves &genMoves) {
    ...
    if (probeRootTB(&bestMove, genMoves)) {
      string best = string(genMoves.decodeBoardinv(&bestMove, genMoves.sideToMove));
  ...
  static void printWdlSyzygy(GenMoves &genMoves) {
    ...
    SYZYGY *syzygy = &SYZYGY::getInstance();
    u64 friends =
        genMoves.sideToMove == WHITE ? board::getBitmap<WHITE>(chessboard) : board::getBitmap<BLACK>(chessboard);
    u64 enemies =
        genMoves.sideToMove == BLACK ? board::getBitmap<WHITE>(chessboard) : board::getBitmap<BLACK>(chessboard);
  ...
  static void printDtzSyzygy(GenMoves &genMoves) {
    genMoves.perftMode = true;
    SYZYGY *syzygy = &SYZYGY::getInstance();
    if ((genMoves.sideToMove == BLACK) ? board::inCheck1<WHITE>(genMoves.chessboard)
                                       : board::inCheck1<BLACK>(genMoves.chessboard)) {
  ...
  static int printDtmWdlGtb(GenMoves &genMoves, const bool dtm) {
    ...
    u64 friends = genMoves.sideToMove == WHITE ? board::getBitmap<WHITE>(genMoves.chessboard)
                                               : board::getBitmap<BLACK>(genMoves.chessboard);
    u64 enemies = genMoves.sideToMove == BLACK ? board::getBitmap<WHITE>(genMoves.chessboard)
                                               : board::getBitmap<BLACK>(genMoves.chessboard);
    genMoves.display();
    cout << "current: ";
    unsigned pliestomate;
    const int res = GTB::getInstance().getDtmWdl(GTB_STM(genMoves.sideToMove), 2, genMoves.chessboard, &pliestomate,
                                                 dtm, genMoves.rightCastle);
    genMoves.incListId();
    genMoves.generateCaptures(genMoves.sideToMove, enemies, friends);
    genMoves.generateMoves(genMoves.sideToMove, friends | enemies);
    _Tmove *move;
