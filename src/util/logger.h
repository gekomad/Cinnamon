...
class Logger : public Singleton<Logger>, public ofstream {
  ...
  public:
    ...
    template <LOG_LEVEL type, typename T, typename... Args> void _log(T t, Args... args) {
  ...
  private:
    ...
    template <typename T> void __log(T t) {
        cout << (t);
        *this << (t);
    }

    template <typename T, typename... Args> void __log(T t, Args... args) {
        cout << (t);
        *this << (t);
        __log(args...);
    }
