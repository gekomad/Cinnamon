...
class Time {
 ...
 public:
  ...
  Time(int latency) { this->latency = latency; }
  ...
  inline void incCount(const string &subName) { this->subName[subName]++; }
  ...
  inline void stop() {
    _totTime += std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now() - _start).count();
    _totTime -= latency;
  }
