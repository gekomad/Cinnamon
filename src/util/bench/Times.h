 ...
 public:
  ...
  void print() {
    ...
    for (auto it = times.begin(); it != times.end(); ++it) {
      ...
      cout << "info string bench " << it->first << setw(30 - it->first.length()) << countP << m << " times\t"
           << "avg ns:" << "\t" << avg1.second << flush;
  ...
  inline void subProcess(const string &name, const string &subName) { times[name]->incCount(subName); }
