...
class Times {
  ...
  public:
    ...
    void print() {
        int64_t tot = 0;
        for (auto & time : times) {
            const auto count1 = time.second->getCount();
            const auto avg1 = avg(time.first);
        ...
        for (auto & time : times) {
            ...
            cout << "info string bench " << time.first << setw(30 - time.first.length()) << countP << m << " times\t" << "avg ns:" << "\t"
                 << avg1.second << flush;
            if (avg1.first != avg1.second) cout << (avg1.second > 1000 ? "\t" : "\t\t") << "without subprocess: " << avg1.first << flush;
            ...
            if (avg1.first != avg1.second) cout << "\t\tTOT: " << a << m << "\t" << (aa * 100 / tot) << "%" << flush;
            else
    ...
    void subProcess(const string &name, const string &subName) {
