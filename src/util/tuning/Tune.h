 ...
 protected:
  ...
  void saveParams(const array<PARAMS, N_PARAM> &params) {
    ...
    for (auto &param : params) {
      myfile << param.name << "=" << searchManager.getParameter(param.name, param.phase) << "|" << param.phase << endl;
    }
    myfile.close();
  }
  ...
  void tune(const set<FEN *> &fens) {
    ...
    const array<PARAMS, N_PARAM> params{
        PARAMS("MOB_KING_", searchManager, Eval::MG), PARAMS("MOB_KNIGHT_", searchManager, Eval::MG),
        PARAMS("MOB_BISHOP_", searchManager, Eval::MG), PARAMS("MOB_QUEEN_", searchManager, Eval::MG),
        //                PARAMS("ATTACK_KING", searchManager, Eval::MG),
        //                PARAMS("BISHOP_ON_QUEEN", searchManager, Eval::MG),
        PARAMS("BACKWARD_PAWN", searchManager, Eval::MG), PARAMS("DOUBLED_ISOLATED_PAWNS", searchManager, Eval::MG),
        PARAMS("PAWN_IN_7TH", searchManager, Eval::MG), PARAMS("PAWN_IN_PROMOTION", searchManager, Eval::MG),
        PARAMS("PAWN_NEAR_KING", searchManager, Eval::MG), PARAMS("PAWN_BLOCKED", searchManager, Eval::MG),
        PARAMS("UNPROTECTED_PAWNS", searchManager, Eval::MG), PARAMS("FRIEND_NEAR_KING", searchManager, Eval::MG),
        PARAMS("BONUS2BISHOP", searchManager, Eval::MG), PARAMS("BISHOP_PAWN_ON_SAME_COLOR", searchManager, Eval::MG),
        PARAMS("OPEN_FILE_Q", searchManager, Eval::MG), PARAMS("ROOK_7TH_RANK", searchManager, Eval::MG),
        PARAMS("KNIGHT_PINNED", searchManager, Eval::MG), PARAMS("ROOK_PINNED", searchManager, Eval::MG),
        PARAMS("BISHOP_PINNED", searchManager, Eval::MG), PARAMS("ROOK_IN_7_KING_IN_8", searchManager, Eval::MG),
        PARAMS("PAWN_PASSED_", searchManager, Eval::MG), PARAMS("DISTANCE_KING_ENDING_", searchManager, Eval::MG),
        ...
        PARAMS("MOB_KING_", searchManager, Eval::EG), PARAMS("MOB_KNIGHT_", searchManager, Eval::EG),
        PARAMS("MOB_BISHOP_", searchManager, Eval::EG), PARAMS("MOB_QUEEN_", searchManager, Eval::EG),
        //                PARAMS("ATTACK_KING", searchManager, Eval::EG),
        //                PARAMS("BISHOP_ON_QUEEN", searchManager, Eval::EG),
        PARAMS("BACKWARD_PAWN", searchManager, Eval::EG), PARAMS("DOUBLED_ISOLATED_PAWNS", searchManager, Eval::EG),
        PARAMS("PAWN_IN_7TH", searchManager, Eval::EG), PARAMS("PAWN_IN_PROMOTION", searchManager, Eval::EG),
        PARAMS("PAWN_NEAR_KING", searchManager, Eval::EG), PARAMS("PAWN_BLOCKED", searchManager, Eval::EG),
        PARAMS("UNPROTECTED_PAWNS", searchManager, Eval::EG), PARAMS("FRIEND_NEAR_KING", searchManager, Eval::EG),
        PARAMS("BONUS2BISHOP", searchManager, Eval::EG), PARAMS("BISHOP_PAWN_ON_SAME_COLOR", searchManager, Eval::EG),
        PARAMS("OPEN_FILE_Q", searchManager, Eval::EG), PARAMS("ROOK_7TH_RANK", searchManager, Eval::EG),
        PARAMS("KNIGHT_PINNED", searchManager, Eval::EG), PARAMS("ROOK_PINNED", searchManager, Eval::EG),
        PARAMS("BISHOP_PINNED", searchManager, Eval::EG), PARAMS("ROOK_IN_7_KING_IN_8", searchManager, Eval::EG)
    ...
    do {
      ...
      for (auto &param : params) {
        ...
        for (int dir = 0; dir < 2; dir++) {
          ...
          int newValue;
          if (dir == 0)
            newValue = searchManager.getParameter(param.name, param.phase) + 1;
          else {
            if (searchManager.getParameter(param.name, param.phase) <= 0)
              continue;
            else
              newValue = searchManager.getParameter(param.name, param.phase) - 1;
          }
          ...
          while (true) {
            ...
            cout << param.name << "(" << (param.phase ? "EG" : "MG") << ") try value: " << newValue
                 << "\terror: " << currentError << "\tbestError: " << bestError;
