  ...
  if (depth == 1) {
    ...
    if (tmp <= valWindow - VAL_WINDOW || tmp >= valWindow + VAL_WINDOW) {
      if (tmp <= valWindow - VAL_WINDOW) {
        tmp = search<side, searchMoves>(depth, valWindow - VAL_WINDOW * 2, valWindow + VAL_WINDOW, &pvLine, nPieces);
      } else {
        tmp = search<side, searchMoves>(depth, valWindow - VAL_WINDOW, valWindow + VAL_WINDOW * 2, &pvLine, nPieces);
      }
      if (tmp <= valWindow - VAL_WINDOW || tmp >= valWindow + VAL_WINDOW) {
        if (tmp <= valWindow - VAL_WINDOW) {
          tmp = search<side, searchMoves>(depth, valWindow - VAL_WINDOW * 4, valWindow + VAL_WINDOW, &pvLine, nPieces);
        } else {
          tmp = search<side, searchMoves>(depth, valWindow - VAL_WINDOW, valWindow + VAL_WINDOW * 4, &pvLine, nPieces);
        }
        if (tmp <= valWindow - VAL_WINDOW || tmp >= valWindow + VAL_WINDOW) {
...
Search::Search() : ponder(false), nullSearch(false) { DEBUG(eval.lazyEvalCuts = cumulativeMovesCount = totGen = 0) }

void Search::clone(const Search *s) { memcpy(chessboard, s->chessboard, sizeof(_Tchessboard)); }
...
void Search::setPonder(const bool r) { ponder = r; }
...
void Search::setMaxTimeMillsec(const int n) { maxTimeMillsec = n; }
...
int Search::search(const int depth, int alpha, const int beta, _TpvLine *pline, const int N_PIECE) {
  ...
  if (!nullSearch && !pvNode && !isIncheckSide) {
    ...
    if (nDepth == 3) {
      ...
      if (pieces != chessboard[PAWN_BLACK + side] || bitCount(pieces) > 9) nDepth = 1;
    }
    if (depth > nDepth) {
      nullSearch = true;
  ...
  while ((move = getNextMove(&genList[listId], depth, hashItem, first++))) {
    ...
    if (move->promotionPiece == NO_PROMOTION) {
      ...
      if (countMove > 3 && !isIncheckSide && depth >= 3 && move->capturedPiece == SQUARE_EMPTY) {
        val = searchLambda(&newLine, depth + extension - (countMove > 6 ? 3 : 2), -(alpha + 1), -alpha, nullptr);
      }
    }
...
u64 Search::getZobristKey() const { return chessboard[ZOBRISTKEY_IDX]; }

void Search::unsetSearchMoves() { searchMovesVector.clear(); }

void Search::setSearchMoves(const vector<int> &s) { searchMovesVector = s; }
...
void Search::setParameter(const string &p, const int value, const int phase) {
  ...
  //    else if (p == "ENEMY_NEAR_KING")eval.ENEMY_NEAR_KING[phase] = value;
    ...
    eval.FRIEND_NEAR_KING[phase] = value;
  //    else if (p == "HALF_OPEN_FILE_Q")eval.HALF_OPEN_FILE_Q[phase] = value;
  else if (p == "BONUS2BISHOP")
    eval.BONUS2BISHOP[phase] = value;
  else if (p == "BISHOP_PAWN_ON_SAME_COLOR")
    eval.BISHOP_PAWN_ON_SAME_COLOR[phase] = value;
  ...
  //    else if (p == "OPEN_FILE")eval.OPEN_FILE[phase] = value;
    ...
    eval.OPEN_FILE_Q[phase] = value;
  else if (p == "ROOK_7TH_RANK")
    eval.ROOK_7TH_RANK[phase] = value;
  ...
  else if (p == "KNIGHT_PINNED")
    eval.KNIGHT_PINNED[phase] = value;
  else if (p == "ROOK_PINNED")
    eval.ROOK_PINNED[phase] = value;
  else if (p == "BISHOP_PINNED")
    eval.BISHOP_PINNED[phase] = value;
  else if (p == "ROOK_IN_7_KING_IN_8")
    eval.ROOK_IN_7_KING_IN_8[phase] = value;
  else if (p == "MOB_QUEEN_")
    eval.MOB_QUEEN_[phase] = value;
  else if (p == "MOB_ROOK_")
    eval.MOB_ROOK_[phase] = value;
  else if (p == "MOB_BISHOP_")
    eval.MOB_BISHOP_[phase] = value;
  else if (p == "MOB_KNIGHT_")
    eval.MOB_KNIGHT_[phase] = value;
  else if (p == "MOB_KING_")
    eval.MOB_KING_[phase] = value;
  else if (p == "PAWN_PASSED_")
    eval.PAWN_PASSED_ = value;
  else if (p == "DISTANCE_KING_ENDING_")
    eval.DISTANCE_KING_ENDING_ = value;
  else if (p == "DISTANCE_KING_OPENING_")
    eval.DISTANCE_KING_OPENING_ = value;
