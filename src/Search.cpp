...
//    if (p == "DOUBLED_PAWNS")return eval.DOUBLED_PAWNS[idx];
    if (p == "PAWN_IN_7TH")return eval.PAWN_IN_7TH[idx];
//    if (p == "PAWN_CENTER")return eval.PAWN_CENTER[idx];
    if (p == "PAWN_IN_PROMOTION")return eval.PAWN_IN_PROMOTION[idx];
//    if (p == "PAWN_ISOLATED")return eval.PAWN_ISOLATED[idx];
    if (p == "PAWN_NEAR_KING")return eval.PAWN_NEAR_KING[idx];
    if (p == "PAWN_BLOCKED")return eval.PAWN_BLOCKED[idx];
    if (p == "UNPROTECTED_PAWNS")return eval.UNPROTECTED_PAWNS[idx];
//    if (p == "ENEMY_NEAR_KING")return eval.ENEMY_NEAR_KING[idx];
    if (p == "FRIEND_NEAR_KING")return eval.FRIEND_NEAR_KING[idx];
//    if (p == "HALF_OPEN_FILE_Q")return eval.HALF_OPEN_FILE_Q[idx];
    if (p == "BONUS2BISHOP")return eval.BONUS2BISHOP[idx];
    if (p == "BISHOP_PAWN_ON_SAME_COLOR")return eval.BISHOP_PAWN_ON_SAME_COLOR[idx];
//    if (p == "CONNECTED_ROOKS")return eval.CONNECTED_ROOKS[idx];
//    if (p == "OPEN_FILE")return eval.OPEN_FILE[idx];
    if (p == "OPEN_FILE_Q")return eval.OPEN_FILE_Q[idx];
    if (p == "ROOK_7TH_RANK")return eval.ROOK_7TH_RANK[idx];
//    if (p == "ROOK_BLOCKED")return eval.ROOK_BLOCKED[idx];
//    if (p == "ROOK_TRAPPED")return eval.ROOK_TRAPPED[idx];
//    if (p == "UNDEVELOPED_KNIGHT")return eval.UNDEVELOPED_KNIGHT[idx];
//    if (p == "UNDEVELOPED_BISHOP")return eval.UNDEVELOPED_BISHOP[idx];
    if (p == "KNIGHT_PINNED")return eval.KNIGHT_PINNED[idx];
    if (p == "ROOK_PINNED")return eval.ROOK_PINNED[idx];
    if (p == "BISHOP_PINNED")return eval.BISHOP_PINNED[idx];
    if (p == "QUEEN_PINNED")return eval.QUEEN_PINNED[idx];
//    if (p == "PAWN_PINNED")return eval.PAWN_PINNED[idx];
    fatal("Not found ", p)
    exit(1);
}
