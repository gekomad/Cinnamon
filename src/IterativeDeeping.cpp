...
IterativeDeeping::IterativeDeeping(const shared_ptr<Hash>& hash, const shared_ptr<SearchManager>& searchManager)
    : maxDepth(MAX_PLY), running(false), ponderEnabled(false) {
  setId(-1);
  this->hash = hash;
...
void IterativeDeeping::setMaxDepth(const int d) { maxDepth = min(d, MAX_PLY); }

IterativeDeeping::~IterativeDeeping() {}

void IterativeDeeping::enablePonder(const bool b) { ponderEnabled = b; }

bool IterativeDeeping::getPonderEnabled() const { return ponderEnabled; }
...
int IterativeDeeping::loadFen(const string& fen) { return searchManager->loadFen(fen); }
