...
class GenMoves : public ChessBoard {
 public:
  ...
  __attribute__((always_inline)) static bool isAttacked(const _Tmove &move, const _Tchessboard &chessboard,
                                                        const u64 allpieces) {
  ...
  bool getForceCheck() const { return forceCheck; }

  void setForceCheck(const bool b) { forceCheck = b; }
  ...
  __attribute__((always_inline)) bool performKingShiftCapture(const u64 enemies, const bool isCapture) {
    ...
    for (u64 x1 = enemies & NEAR_MASK1[pos]; x1; RESET_LSB(x1)) {
      ...
      if (pushmove<STANDARD_MOVE_MASK, side>(pos, BITScanForward(x1), NO_PROMOTION, KING_BLACK + side, isCapture)) {
        return true;
      }
    }
    return false;
  ...
  __attribute__((always_inline)) bool performKnightShiftCapture(const u64 enemies, const bool isCapture) {
    ...
    for (u64 x = chessboard[KNIGHT_BLACK + side]; x; RESET_LSB(x)) {
      ...
      for (u64 x1 = enemies & KNIGHT_MASK[pos]; x1; RESET_LSB(x1)) {
        ...
        if (pushmove<STANDARD_MOVE_MASK, side>(pos, BITScanForward(x1), NO_PROMOTION, KNIGHT_BLACK + side, isCapture))
          return true;
      }
    }
    return false;
  ...
  __attribute__((always_inline)) bool performDiagCapture(const uchar piece, const u64 enemies, const u64 allpieces) {
    ...
    for (u64 x2 = chessboard[piece]; x2; RESET_LSB(x2)) {
      ...
      for (; diag; RESET_LSB(diag)) {
        ...
        if (pushmove<STANDARD_MOVE_MASK, side>(position, BITScanForward(diag), NO_PROMOTION, piece, true)) return true;
      }
    }
    return false;
  }
  ...
  template <uchar side>
  __attribute__((always_inline)) bool performRankFileCapture(const uchar piece, const u64 enemies,
                                                             const u64 allpieces) {
  ...
  __attribute__((always_inline)) bool performPawnCapture(const u64 enemies) {
    ...
    if (enPassant != NO_ENPASSANT) {
      ...
      for (; x; RESET_LSB(x)) {
        const int o = BITScanForward(x);
        BENCH_SUBPROCESS("pawnCapture", "pushmove")
        pushmove<ENPASSANT_MOVE_MASK, side>(o, (side ? enPassant + 8 : enPassant - 8), NO_PROMOTION, side, true);
      }
      updateZobristKey(ENPASSANT_RAND, enPassant);
      enPassant = NO_ENPASSANT;
    }
  ...
  int getListSize() const { return genList[listId].size; }
 ...
 protected:
  ...
  bool __attribute__((always_inline)) inCheckSlow(const int from, const int to, const uchar pieceFrom,
                                                  const uchar pieceTo, const uchar promotionPiece) {
    bool result;
    switch (type & 0x3) {
      case STANDARD_MOVE_MASK: {
        ...
        result = board::isAttacked<side>(BITScanForward(chessboard[KING_BLACK + side]),
                                         board::getBitmap<BLACK>(chessboard) | board::getBitmap<WHITE>(chessboard),
                                         chessboard);
  ...
  __attribute__((always_inline)) bool inCheck(const uchar from, const uchar to, const uchar pieceFrom,
                                              const uchar pieceTo, uchar promotionPiece) {
  ...
  __attribute__((always_inline)) void tryAllCastle(const u64 allpieces) {
    ...
    if (chess960)
      tryAllCastle960<side>(allpieces);
    else
  ...
  bool __attribute__((always_inline)) pushmove(const uchar from, const uchar to, const uchar promotionPiece,
                                               const uchar pieceFrom, const bool isCapture) {
  ...
  _Tmove *getMove(const int i) const { return &genList[listId].moveList[i]; }

  virtual void setRunning(const int t) { running = t; }

  virtual int getRunning() const { return running; }
