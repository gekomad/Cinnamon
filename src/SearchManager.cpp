...
SearchManager::SearchManager() { threadPool = new ThreadPool<Search>(1); }
...
int SearchManager::getForceCheck() { return threadPool->getThread(0).getForceCheck(); }

u64 SearchManager::getZobristKey(const int id) { return threadPool->getThread(id).getZobristKey(); }
...
int SearchManager::getScore(const uchar side) { return threadPool->getThread(0).getScore(side); }

int SearchManager::getMaxTimeMillsec() { return threadPool->getThread(0).getMaxTimeMillsec(); }
...
void SearchManager::updateFenString() { threadPool->getThread(0).updateFenString(); }
...
string SearchManager::decodeBoardinv(const _Tmove *move) { return threadPool->getThread(0).decodeBoardinv(move); }
...
int SearchManager::printDtmGtb(const bool dtm) { return TB::printDtmWdlGtb(threadPool->getThread(0), dtm); }

void SearchManager::printDtmSyzygy() { TB::printDtzSyzygy(threadPool->getThread(0)); }

void SearchManager::printWdlSyzygy() { TB::printWdlSyzygy(threadPool->getThread(0)); }
...
bool SearchManager::setNthread(const int nthread) { return threadPool->setNthread(nthread); }
