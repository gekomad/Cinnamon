...
void
Tablebase::print ( unsigned stm1, unsigned info1, unsigned pliestomate1 ) {
    if ( info1 == tb_DRAW ) {
        cout << "Draw";
...
bool
Tablebase::setInstalledPieces ( int n ) {
    if ( n < 3 || n > 5 ) {
        return false;
    }
...
void
Tablebase::setPath ( string path1 ) {
    path = path1;
    load (  );
}
