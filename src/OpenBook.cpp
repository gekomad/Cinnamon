...
u64
OpenBook::createKey ( string fen1 ) {
    ...
    unsigned p;
    u64 key = 0;
    sscanf ( fen, "%72s %c %4s %2s", board_s, &to_move_c, castle_flags_s, ep_square_s );
    r = 7;
    f = 0;
    p = 0;
...
int
OpenBook::intFromFile ( int l, u64 * r ) {
    int c;
...
int
OpenBook::entryFromFile ( entry_t * entry ) {
    u64 r = 0;
...
int
OpenBook::findKey ( u64 key, entry_t * entry ) {
...
void
OpenBook::moveToString ( char move_s[6], unsigned short move ) {
